...
Sure, I'll refine the description for each type of variable and the
rules of thumb for added clarity and accuracy.
...
\item
  \textbf{Remember the Qualitative Nature of DAGs}: Directed Acyclic
  Graphs (DAGs) are qualitative representations. We use strategies such
  as time stamps to maintain chronological order rather than indicate
  precise units of time. Again we aim for simplicity. Include only as
  much detail as is required to clarify strategies for confounding
  control.
\end{enumerate}
...
that also affects \(Y\). Conditioning on \(L\) in this scenario evokes
...
For the time being, let us set aside the intricate questions arising
from complex longitudinal dynamics. Next we discuss how temporally
ordered causal diagrams can elucidate the usefulness of a three-wave
panel design for tackling causal questions using data. We consider the
simplest design for longitudinal data collection that allows for causal
estimation. Additionally, with three measurement intervals, evolutionary
anthropologists have the opportunity to collect current data that can
later inform future causal investigations. To further illuminate the
potential of this method, I will revisit the question of how religion
influences behavior.''
