...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.1395}}
...
on the time varying confounder. Damned either way. As with mediation,
however, it may be possible to simulate counterfactual contrasts by
fixing exposures to specific levels, and asking: ``What would happen if
\ldots?'' Models for assessing the causal effects of time-fixed and
time-varying exposures belong to a class of methods called
``G-methods.'' There has been tremendous development in the applications
of these methods in the health sciences {[}CITE TMLE papers.{]} However,
such methods are not to my knowledge widely employed by cultural
evolutionary researchers. As mentioned in Part 1., we cannot attempt
causal inference without satisifying the three fundamental assumptions
of causal inference. Also as mentioned, such assumptions would appear
too strict for topics in cultural evolution pertaining to the historical
past. For now, it is critical to understand that specialist tools such
as G-methods are required to address many causal questions because
treatment-confounder feedbacks means that we cannot identify many causal
effects from manifest data.
...
with the world as it might have been. For many big questions
confounder-treatment feedback lead to identification problems. Causal
diagrammes clarify the pressure points. I next turn to three-wave
designs for estimating the total causal effects. Such designs have
applications for a broad class of cultural evolutionary questions, and
may be especially useful for evolutionary anthropologists who wish to
collect time-series data in the present to address causality.

In this section, I use causal diagrammes to clarify how a three-wave
panel design may be useful. Again, temporally ordered causal diagrammes
will help us to discover how to collect and model longitudinal data to
...
greater donations of money and time.
...
For example: ``Self-reported monthly church attendance''
...
For example: ``Charitable giving as measured by self-reports'';
``Volunteering as measured by self-reports.''
