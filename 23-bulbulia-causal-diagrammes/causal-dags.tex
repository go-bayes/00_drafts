...
\hypersetup{
  pdftitle={Causal Diagrams (DAGS) for Evolutionary Human Science: A Practical Guide},
...
(\protect\hyperlink{ref-hernuxe1n2016}{Hernán \emph{et al.} 2016}). In
...
counterfactual reasoning. We must contrast the world as it is when \(A\)
is set to one level with the world as it might have been when \(A\) is
set to another level. Our objective in causal inference is to quantify
the magnitude of such contrasts.
...
\item
  ...
  \(A_i = 1\).\footnote{The counterfactual outcome under exposure
    \(A_i = a\) can be denoted in several ways, such as \(Y_i(a)\),
    \(Y_i^{a}\), and \(Y_a\) (without a subscript). Here, we use the
    notation \(Y(a)\), dropping the subscript \(i\) for clarity.}
\end{itemize}

To compute an individual causal effect, we contrast two potential
outcomes for the same unit, representing two different states of the
world. Since each individual can be observed under one treatment
condition at a time, one of these states is inevitably counterfactual.
...
\item
  If
  \(A = 14, the outcome under treatment is observed (\)Y\_i\textbar A=1\(), but the outcome under no treatment is unobserved (\)Y\_i\textbar A=0\$).
  This can be expressed as:

  \[ Y_i|A = 1 \implies Y_i|A=0~ \text{is counterfactual} \]
\item
  Conversely, if ( A = 0 ), the outcome under no treatment is observed
  (\(Y_i|A=0\)), but the outcome under treatment is unobserved
  (\(Y_i|A=1\)). This can be expressed as:

  \[ Y_i|A = 0 \implies Y_i|A=1~ \text{is counterfactual} \]
\end{itemize}

This framework highlights the fundamental challenge of causal inference,
where one of the potential outcomes is always missing. That individual
causal effects are not identified from observations is ``\emph{the
fundamental problem of causal inference}''
(\protect\hyperlink{ref-holland1986}{Holland 1986};
\protect\hyperlink{ref-rubin1976}{Rubin 1976}).

Next, we consider assumptions under which average causal effects may be
identified from the data. For now, it is important to note that because
average causal effects require summing over individual causal effects,
causal inference must inevitably solve a special, elusive, \emph{missing
data problem} (\protect\hyperlink{ref-edwards2015}{Edwards \emph{et al.}
...
of assumptions.\footnote{There are different formulations of causality,
  ...
  Neyman (\protect\hyperlink{ref-neyman1923}{\textbf{neyman1923?}}),
  Rubin Rubin (\protect\hyperlink{ref-rubin1976}{1976}), and Robins
  Robins (\protect\hyperlink{ref-robins1986}{1986}).}
...
inspection, however, the requirement for treatment variation irrelevance
sets a high bar.
...
\protect\hyperlink{ref-hernuxe1n2016}{Hernán \emph{et al.} 2016};
...
(\protect\hyperlink{ref-keogh2020}{Keogh \emph{et al.} 2020})).
...
{\centering \includegraphics[width=1\textwidth,height=\textheight]{causal-dags_files/figure-pdf/fig-dag-measure-selection-0-1.pdf}
...
{\centering \includegraphics[width=1\textwidth,height=\textheight]{causal-dags_files/figure-pdf/fig-dag-measure-selection-1.pdf}
...
(\protect\hyperlink{ref-bandalos2018}{2018}). Additionally, specialist
knowledge may often indicate the direction of influence in the
associations of measurement error terms, whether positive or negative
...
\protect\hyperlink{ref-vanderweele2010}{2010}). In such cases,
researchers may sometimes employ causal diagrams with signed paths to
improve causal inferences
(\protect\hyperlink{ref-vanderweele2012}{VanderWeele and Hernán 2012}).
Such methods take us beyond the purpose of this section, which has been
to review how causal diagrams can be used to reveal sources of
