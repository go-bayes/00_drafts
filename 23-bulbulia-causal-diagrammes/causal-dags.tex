...
Also as mentioned, such assumptions would appear too strict for topics
in cultural evolution pertianing to the historical past. In any event,
it is critical to understand that the specialist tools such as G-methods
are required to address many causal questions because
...
\item
  ...
  outcome, are that might be the effect of such a factor. Note that we
  remove instrumental variables**: next, take out any factors that are
  known to be `instrumental variables'. These are factors that cause the
  exposure but do not affect the outcome. Including instrumental
  variables reduces efficiency.
\item
