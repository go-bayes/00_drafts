...
### Advice: Consider the Temporal Sequence of All Measured Variables
...
To manage the issue of conditioning on a common effect, it is crucial to maintain the correct temporal order:

1. Measure all confounders $L_{t0}$, which are common causes of both the exposure $A_{t1}$ and the outcome $Y_{t2}$.
2. Ensure that $A_{t1}$ is measured before $Y_{t2}$.
...
### M-bias: Conditioning on a Collider Before the Exposure May Introduce Bias
...
**This scenario reveals that thoughlessly following a simple rule that requires us to condition only on pre-exposure (and indeed pre-outcome) variables would be hasty.** More generally, demonstrates the imperative for thinking carefully about data collection. Each problem must be approached anew.
