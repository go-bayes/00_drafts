...
### Objective 
 ...
 Correlation is not causation. However, persisten confusion in the analysis and reporting of correlation across many human sciences has given rise to a *causal crisis*. Despite the potential of causal diagrams (DAGs) to address this predicament, their practicality is sometimes obscured by improper use. For example, with only cross sectional data, it is generally difficult to justify the assumptions required for causal inference. Cross-sectional researchers may be tempted to employ causal diagrammes to clarify their assumptions. However, where are assumptions are strong, the appropriate reponse is to pause and collect appropriate time-series data. By reporting results that condition on strong assumptions researchers armed with DAGs perpetuate the *causal crisis*. Causal diagrammes that are structured such that the temporal sequence of confounder, cause, and effect is reflected in the spatial organisation of the graph may greatly assist researchers in understanding understanding the assumption required to consistently estimate unbias causal effects. 
 
Here, my primary objective is to deliver a comprehensive tutorial that showcases the practical utility of temporally organised causal diagrams for obataining a more precise and informed approach to discerning causality.

**Part 1.** Develop the connection between causal diagrammes and the potential outcomes framework.

**Part 2.** Reviews the four fundamental graphs that form the basis of causal understanding. 

**Part 3.** Applies the time graph to a substantative problem in measurement.
...
When it comes to causal estimation, correlation is not the problem. The problem is bias. We say a relation between an exposure $A$ and and outcome $Y$ is causal if the corration between $A$ and $Y$ is unbiased.  What, then, is bias? 
