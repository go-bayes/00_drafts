...
We are entitled to claim that $A$ causes $Y$ if altering $A$ would have influenced the outcome of $Y$ [@hume1902; @lewis1973]. This claim requires counterfactual reasoning. We must contrast the world as it is when $A$ is set to one level with the world as it might have been when $A$ is set to another level. Our objective in causal inference is to quantify the magnitude of such contrasts.
...
[^3]: The counterfactual outcome under exposure $A_i = a$ can be denoted in several ways, such as $Y_i(a)$, $Y_i^{a}$, and $Y_a$ (without a subscript). Here, we use the notation $Y(a)$, dropping the subscript $i$ for clarity.

To compute an individual causal effect, we contrast two potential outcomes for the same unit, representing two different states of the world. Since each individual can be observed under one treatment condition at a time, one of these states is inevitably counterfactual.

For a binary treatment $A$:

-   If $A = 14, the outcome under treatment is observed ($Y_i\|A=1$), but the outcome under no treatment is unobserved ($Y_i\|A=0\$). This can be expressed as:

    $$ Y_i|A = 1 \implies Y_i|A=0~ \text{is counterfactual} $$

-   Conversely, if ( A = 0 ), the outcome under no treatment is observed ($Y_i|A=0$), but the outcome under treatment is unobserved ($Y_i|A=1$). This can be expressed as:

    $$ Y_i|A = 0 \implies Y_i|A=1~ \text{is counterfactual} $$

This framework highlights the fundamental challenge of causal inference, where one of the potential outcomes is always missing. That individual causal effects are not identified from observations is "*the fundamental problem of causal inference*" [@rubin1976; @holland1986].

Next, we consider assumptions under which average causal effects may be identified from the data. For now, it is important to note that because average causal effects require summing over individual causal effects, causal inference must inevitably solve a special, elusive, *missing data problem* [@westreich2015; @edwards2015].
