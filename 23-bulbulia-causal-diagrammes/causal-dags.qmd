...
To mitigate the issue of mediator bias, particularly when our focus is on total effects, we should avoid conditioning on a mediator. This can be achieved by ensuring that the mediator $L$ takes place before the treatment $A$ and the outcome $Y$. This underlines the importance of explicitly stating the temporal ordering of our variables. By including time indexing of all variables in our causal diagramme, and by clearly labelling mediators (e.g. with $M_t$), we reduce the potential for mediator bias from over-conditioning.[^10]
...
Observe that only one counterfactual outcome requires evaluation when we are assessing effect modification, i.e., \$E\[Y(a\*)\|L, G = g\]-E\[Y(a)\|L, G = g\]\$. It is important to remember that **causal graphs are not parametric, so to represent interaction, do *not* draw a path intersecting another path**.
