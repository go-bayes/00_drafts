...
abstract: | 
  This article provides practical advice for creating causal diagrams. It recommends aligning a graph's spatial layout with causation's temporal order. I begin by reviewing assumptions foundational to causal inference. I then explain how causal diagrams may be used to uncover structurally sources of bias. I conclude by using chronologically ordered diagrams to clarify widely misunderstood concepts causal interaction, mediation, and dynamic longitudinal growth. Overall, this guide hopes to equip evolutionary human scientists with tools and understanding to enhance the rigour and clarity of their causal inferences.
author: 
...
It is widely known that correlation does not imply causation. Nevertheless, many human scientists report manifest correlations and use hedging language that suggests causation. However, such reporting lacks justification. Making matters worse, widely adopted strategies for confounding control, such as over-adjustment in regression, are know to often enhance bias [@mcelreath2020]. Across many human sciences, persistent confusion in the analysis and reporting of correlations has greatly impeded scientific progress, resulting in what might be called a "causality crisis" [@bulbulia2022].

Incorporating causal diagrams (also called "directed acyclic graphs" or "DAGs"), into methodologically sound workflows can help to address this causality crisis.[^1]. A system of formal mathematical proofs underpin their operation, bringing confidence. They do not require extensive mathematical training making them broadly accessible. They rely on visualisation, making them user-friendly for the sighted.
...
Yet, causal inference differs from traditional data science by operating within a theoretical framework that requires estimating counterfactual contrasts -- partially hypothetical outcomes under different scenarios from those that have been realised. To estimate quantities from counterfactuals requires something different from ordinary data science. Estimating counterfactual contrasts from the patterns in observed data requires a framework of theory and assumptions that might be called  "full data science" -- "full" in the senses that the data we observe inevitably provide only partial insights into causality. We will review this framework of theory and assumptions in Part 1. For now, it is important to recognise that causal diagrams obtain their utility within the frameworks of theory and assumptions that have come to define full data science. Their function is to clarify structural sources of bias and assist in understanding conditions in which causal effects may be consistently estimated inferred from observational data, based on a pre-specified causal model and its assumptions. Ultimately, whether causal diagrams are effective depends then on the validity of a causal model and its assumptions. A flawed model or overly optimistic assumptions will result in misplaced confidence. The danger, then, is that casual diagrams may exacerbate the causality crisis. This article provides advice for creating causal diagrams that discourage their users from over-reaching.  


There are many excellent introductions to causal diagrams [@rohrer2018; @hernán2023; @cinelli2022; @barrett2021; @mcelreath2020; @greenland1999; @suzuki2020; @pearl2009].[^2].  The following guide contributes to these excellent efforts by showing that *chronologically ordered causal diagrams*, in which the spatial configuration of the graph is aligned with the temporal sequence of causality, greatly mitigates the risk of overreaching. Moreover, chronologically ordered causal diagrams may better inform researchers at the planning phase of research in data collection.  A third benefit of *chronological hygiene* ih the graph is that it helps to clarify widespread confusions in the analysis of multivariate time-series data, where there is presently great confusion. 
...
**Part 1** introduces the counterfactual framework for causal inference as the appropriate theoretical setting for developing causal diagrams. Before attempting causal diagrams, it is vital that users understand their role in within the larger conceptual frameworks of the "full data science" that supports quantitative counterfactual contrasts. Although the methods of this science formally resemble classical data science, we shall understand that the data do not -- and cannot -- provide sufficient insights into causality. (This is so even in experimental settings, where randomisation provides the assurances.)

**Part 2**  introduces chronologically ordered causal diagrams, focussing on their applications in addressing identification conditions where there is *confounding bias*. Here, we find that chronological hygiene in the composition of a causal diagram clarifies structural sources of bias and clarifies whether their are modelling strategies that may address it. More basic, chronological hygiene but also provides directives for data collection, revealing the importance of collecting repeated measures on units over a time.   

**Part 3** employs chronologically ordered causal diagrams to clarify widely the misunderstood concepts of causal interaction, mediation, and longitudinal growth modelling. Here again, we find that attending to chronology in the spatial layout of the graph, when combined with understanding the fundamentals of counterfactual inference, greatly enhances scientific understanding by allowing us to better ask, and answer, causal questions in settings where confused and confusing statistical modelling traditions currently hold sway.
