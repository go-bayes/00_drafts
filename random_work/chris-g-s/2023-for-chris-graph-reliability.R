...
# read omega data
omega <- read_delim(here::here("random_work","chris-g-s", "Omega.txt"), delim = "\t", locale = locale(encoding = "UTF-8"))
...
s_reminder  <- read_delim(here::here("random_work","chris-g-s", "short-remainder.txt"), delim = "\t", locale = locale(encoding = "UTF-8"))
...
dat$scale <- forcats::fct_rev(dat$scale)
...
dat2$scale <- forcats::fct_rev(dat2$scale)
