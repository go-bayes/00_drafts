---
title: "Causal effects of meaning of life on multi-dimensional well-being"
...
push_mods <-
  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23-ow-fl-meaning-full"
  )
...
hist( dat$lifemeaning)
hist( dat$meaning_purpose )
hist( dat$meaning_sense )


mean( dat$lifemeaning, na.rm = TRUE)
mean( dat$meaning_sense, na.rm = TRUE )
mean( dat$meaning_purpose, na.rm = TRUE)

sd( dat$lifemeaning, na.rm = TRUE)
sd( dat$meaning_sense, na.rm = TRUE )
sd( dat$meaning_purpose, na.rm = TRUE)

table( is.na( dat$meaning_purpose) )
table( is.na( dat$meaning_sense) )
...
dat_long <- dat |>
 ...
 # mutate(religion_believe_spirit_not = abs(religion_believe_spirit - 1)) |> # see target trial for use of this variable
  ...
  dplyr::mutate(meets_criteria_baseline = ifelse(year_measured == 1 &  ! is.na(lifemeaning), 1, 0) )|>
...
# check unique n
n_unique(dat_long$id) # 34745 # sample size

# checks
table(dat_long$wave, dat_long$lifemeaning)
...
table( dt_test$lifemeaning) 
table(dt_test$meaning_sense)

dt_test <- create_ordered_variable(dt_test, "lifemeaning", 2)
dt_test <- create_ordered_variable(dt_test, "meaning_purpose", 2)
dt_test <- create_ordered_variable(dt_test, "lifemeaning", 4)
dt_test <- create_ordered_variable(dt_test, "meaning_purpose", 4)
dt_test <- create_ordered_variable(dt_test, "meaning_sense", 2) # won't work otherwise
dt_test <- create_ordered_variable(dt_test, "meaning_sense", 4) # won't work otherwise

# see the trouble -- the scale is too top-heavy
print(quantile(dt_test$meaning_sense, probs = seq(0, 1, 1/4), na.rm = TRUE))
table(dt_test$meaning_sense_4tile)

# not suited to quartiles 
max(dt_test$meaning_sense, na.rm=TRUE)


table( dt_test$lifemeaning_4tile)
table( dt_test$meaning_purpose_4tile)
table( dt_test$meaning_sense_4tile)

table( dt_test$lifemeaning_2tile)
table( dt_test$meaning_purpose_2tile)
table( dt_test$meaning_sense_2tile)


lifemeaning_labels <- levels( dt_test$lifemeaning_4tile )
lifemeaning_labels

# save labels for use in positivity table
here_save(lifemeaning_labels, "lifemeaning_labels")
...
# 12677     +  5614  +    11774    +   4680 

sd( dat_long_t$lifemeaning, na.rm = TRUE) + mean(dat_long_t$lifemeaning, na.rm = TRUE)
# check positivity
mean(dat_long_t$lifemeaning, na.rm = TRUE)
# make data for positivity
dt_positivity_full <- dat_long_t|>
  filter(wave == 2018 | wave == 2019) |> 
  select(wave, id, lifemeaning_coarsen, lifemeaning) |> 
  mutate(lifemeaning_coarsen_n = as.numeric( lifemeaning_coarsen ))

table( dt_positivity_full$lifemeaning_coarsen_n ) 

# save for use later
here_save(dt_positivity_full, "dt_positivity_full")


hist( dt_positivity_full$lifemeaning_coarsen_n )

out <- msm::statetable.msm(lifemeaning_coarsen_n, id, data = dt_positivity_full)

# transition table
t_tab <- transition_table(out, state_names = lifemeaning_labels)
t_tab
...
str(dat_long_t$lifemeaning_coarsen)

hist( dat_long_t$lifemeaning_coarsen_n)
...
exposure_var = c("lifemeaning", "lifemeaning_coarsen") # save both continuous and categorical
...
# ego related 
# move sexual sat to relationships
# move vengence to reflective
  # "modesty", # I want people to know that I am an important person of high status, I am an ordinary person who is no better than others. , I wouldn’t want people to treat me as though I were superior to them. I think that I am entitled to more respect than the average person is.
  # # "religion_religious", # Do you identify with a religion and/or spiritual group?
...
# assign name
exposure_vars = "t1_lifemeaning_coarsen"
...
exposure_vars_continuous = "t1_lifemeaning_z"
...
levels(prep_health$t1_lifemeaning_coarsen)

# checks
a <- nrow( prep_social_multiple$`[1, 5)`)
b <-nrow( prep_social_multiple$`[5, 5.5)`)
c <- nrow( prep_social_multiple$`[5.5, 6.5)`)
d <-nrow( prep_social_multiple$`[6.5, 7)`)
#prep_social
# check
...
baseline_vars_health = mice_health_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_z)|> colnames() # target trial, randomised baseline
...
dt_health = match_ebal_health_trim

# set treatment
treat_0 = "[1, 5)" # secular
treat_1 = "[5.5, 6.5)" # religious 
...
table(mice_health_long$t1_lifemeaning_z)
...
baseline_vars_health_con = mice_health_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_coarsen)|> colnames() # target trial, randomised baseline
...
# # checks results
# # ebal method
bal.tab(match_ebal_health_cont)
love.plot(match_ebal_health_cont, binary = "std", thresholds = c(m = .1),
          wrap = 50, position = "bottom", size =2)

# consider results
sum_ebal_health_cont <- summary(match_ebal_health_con)
sum_ebal_health_cont
plot(sum_ebal_health_cont)



# For trimmed weights e.g.
#trim if needed (weights > 10 might be a problem)
match_ebal_health_cont_trim  <- WeightIt::trim(match_ebal_health_con, at = .997)
...
sum_cont <- summary(match_ebal_health_cont_trim)
sum_cont
plot(sum_cont)

love.plot(match_energy_health_cont_trim, binary = "std", thresholds = c(m = .1),
          wrap = 50, position = "bottom", size =2, limits = list(m = c(-.5, .5)))



dt_health_con =   match_ebal_health_cont_trim
...
baseline_vars_embodied = mice_embodied_long |>
  dplyr::select(starts_with("t0"), -t0_lifemeaning_z) |> colnames()
...
baseline_vars_embodied_con = mice_embodied_long |>
  dplyr::select(starts_with("t0"), -t0_lifemeaning_coarsen) |> colnames()
...
love.plot(match_ebal_embodied_con, binary = "std", thresholds = c(m = .1),
          wrap = 50, position = "bottom", size =2) 
...
# settings 
dt_embodied_con = match_ebal_embodied_con_trim
...
baseline_vars_practical = mice_practical_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_z)|> colnames() # strange to include these -- as they  are income by other names
...
baseline_vars_practical_con = mice_practical_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_coarsen)|> colnames() # strange to include these -- as they  are income by other names
...
love.plot(match_ebal_practical_con, binary = "std", thresholds = c(m = .1),
          wrap = 50, position = "bottom", size =2, limits = list(m = c(-.5, .5))) 
...
love.plot(match_ebal_practical_con_trim, binary = "std", thresholds = c(m = .1),
          wrap = 50, position = "bottom", size =2, limits = list(m = c(-.5, .5)))
...
# Set DF 
dt_practical_con = match_ebal_practical_con_trim
...
baseline_vars_reflective = mice_reflective_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_z)|> colnames() # no variance at baseline
...
mod_reflective_gratitude  <- double_robust(
  df = dt_reflective, # data
  Y = "t2_gratitude_z", # outcome
  X = X,  # treatment 
  baseline_vars = baseline_vars_reflective,
  treat_1 = treat_1,
  treat_0 = treat_0,
...
baseline_vars_reflective_con = mice_reflective_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_coarsen)|> colnames() # no variance at baseline
...
# if trim is needed
match_ebal_reflective_con_trim <- WeightIt::trim(match_ebal_reflective_con, at = .997)
...
love.plot(match_ebal_reflective_con_trim, binary = "std", thresholds = c(m = .1))
here_save(match_ebal_reflective_con_trim, "match_ebal_reflective_con_trim")
dev.off()
...
dt_reflective_con = match_ebal_reflective_con_trim
...
baseline_vars_social = mice_social_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_z)|> colnames() # no variance in exposure at baseline
...
baseline_vars_social_con = mice_social_long |> 
  dplyr::select(starts_with("t0"), -t0_lifemeaning_coarsen)|> colnames() # no variance in exposure at baseline
...
#if trim is needed
match_ebal_social_con_trim <- WeightIt::trim(match_ebal_social_con, at = .997)
...
love.plot(match_ebal_social_con_trim, binary = "std", thresholds = c(m = .1))
...
Here, we investigate the causal effects of hypothetically intervening to change levels of meaning of life on multi-dimensional well-being. 

We operationalize meaning of life using two items from @steger_meaning_2006:
...
We investigate causal effects across multiple dimensions of well-being, following an "outcomewide approach" [@vanderweele2020].
...
lifemeaning_labels <- here_read( "lifemeaning_labels")

out_con <- msm::statetable.msm(lifemeaning_round, id, data = dt_positivity_full)
...
# transition table
t_tab <- transition_table(out, lifemeaning_labels)
...
: Transition matrix for continuous stability and change in meaning of life (rounded to nearest whole number) {#tbl-transition}
...
: Transition matrix for change stability and change in meaning of life by quartiles  {#tbl-transition-factor}


We also investigate change by grouping the exposure in to quartiles. Doing so has the advantage of clarifying the causal effect of interst.  Here, we contrast the well-being outcomes from a hypothetical change between the lowest quartile (1-5 on the response scale) to the third quartile (5.5-6.5). @tbl-transition-factor presents change when individuals are grouped into categories. The causal question here shifts to one that may be more useful in clinical settings. What would happen if we could move people from the lowest quartile of meaning of life to the second highest quartile? 
...
We use Doubly Robust Estimation to estimate the impact of increased "meaning of life" on people. This method combines two different statistical techniques and is resilient to errors: if one of them goes wrong, the other can still provide accurate results.

The first part of our process is calculating a "propensity score" for each individual. Think of it as the odds of someone experiencing a change in their "meaning of life" based on certain characteristics they have. To get this score, we use a statistical model that estimates these role of the characteristics.

Then, we assign a weight to each person in our study based on their propensity score. Those who have higher odds of experiencing a change get a higher weight and vice versa. This helps balance out our sample so that those with more or less propensity don't dominate our results.
...
Then, we use this outcome model to predict what would happen if everyone in our sample had the same level of change in their "meaning of life", no matter what their actual level of change was. Predicting counterfactual outcomes is the core-engine of causal estimation. 

Finally, we estimate the overall effect of changing the "meaning of life" level by calculating the difference in these predicted outcomes between two specific levels of change. For our study, we looked at two scenarios: one where the "meaning of life" increases by one standard deviation from the average and another where it moves from the lower quartile to the third quartile on a seven-point scale.

We  used simulation methods to calculate our standard errors and confidence intervals, which is the margin of error in which we can confidently say our actual result lies [@greifer2023].
...
This set of results reports the hypothetical effect of intervening on meaning of life by setting the population to average meaning of life and incleasing meaning-of life by +1 standard deviations. Outcomes are measured one-year after this hypothetical intervention. 

## Results for the contiuous model (intervention of +1 SD from average meaning of life)

### Effects on health

As indicate in @fig-results-health_con, the expected + one-year effect of
...
# set title
title = "ATE: +1 year causal effect from lowest to third quartile: meaning of life"
...
### Effects on practical well-being contiuous model (intervention of +1 SD from average meaning of life)
...
@tbl-results-reflective_con

### Effects social well-being continous model (intervention of +1 SD from average meaning of life)
...
This set of results reports the hypothetical effect of intervening on meaning of life by setting the population to the lower quartile of meaning of life, and increasing meaning-of life to the third-quartile. Outcomes are measured one-year after this hypothetical intervention. These results may be useful in clinical applications.

### Effects on health (first to third quartile intevention on meaning of life)
...
### Effects on embodied well-being (first to third quartile intevention on meaning of life)
```{r}
...
### Effects on practical well-being (first to third quartile intevention on meaning of life)
...
ggsave(
  ...
  height = 6,
  units = "in",
  filename = "group_plot_ate_practical.jpg",
  device = 'jpeg',
  limitsize = FALSE,
  dpi = 600
)
...
### Effects on reflective well-being (first to third quartile intevention on meaning of life)
...
### Effects social well-being (first to third quartile intevention on meaning of life)
...
#   height = 9,
#   units = "in",
#   filename = "meaning-group_plot_ate_social",
#   device = 'jpeg',
#   limitsize = FALSE,
#   dpi = 600
# )
...
Here, we combined rigorous methods from causal epidemiology with national scale time-series data to estimate the causal effects of meaning of life on multidimensional well-being. We used doubly robust methods that combine propensity score weights with regression stratification. By controlling for measures of all outcomes at baseline we reduce the probability of unmeasured confounding. Because this cannot be ensured, we report E-values, a sensitivity analysis that clarifies the "worst case" scenario for an unmeasured confounder to explain away the results.

**Health domain**: The expected +1 year effect...

Note it is interesting that meaning of life should be causally associated with increasing belief that I will get sick.  This suggests (realism?)

**Embodied well-being domains**:The expected +1 year effect...

**Practical well-being**: The expected +1 year effect...

**Reflective well-being**: The expected +1 year effect ...

**Social well-being-being**: The expected +1 year effect ...
