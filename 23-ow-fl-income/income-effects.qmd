...
dat_long <- dat |>
 ...
 # mutate(eth_cat = as.integer(as.factor(eth_cat))) |>
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(household_inc) &
                                 household_inc > 30975, 1, 0)) |>   # creating an indicator for the first wave
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
  dplyr::mutate(k_19 =  ifelse(
    wave == 2019 &
      year_measured == 1 &
      !is.na(household_inc) == 1 &
      household_inc > 30975,
    1,
    0
  )) |>   # creating an indicator for the first wave; note that we allow people t
...
table( mice_health_long$t0_rural_gch2018) # sparce urban
...
sim.imp <- misim(fits, n = 50, vcov = "HC3")

sim.att <- sim_ame(sim.imp, var = X,
                   subset = t1_household_inc_quantile == treat_1, cl = 3,
                   verbose = TRUE)


est_1 <- transform(sim.att, RD = `E[Y(q4)]` - `E[Y(q1)]`)
