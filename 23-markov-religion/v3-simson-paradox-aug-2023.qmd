...
plotmat(church_mat, pos = c(2, 1), cex.txt = 0.8)
...
# 
# # create edges
# edges <- as.data.frame(as.table(church_mat))
# 
# # graph object
...
# 
# # size
# size_values <- c(0.85, 0.15)
# 
# g <- set_vertex_attr(g, "size", value = size_values)
# 
# g <- set_vertex_attr(g, "color", value = ifelse(size_values < mean(size_values), "red", "blue"))
# layout <- create_layout(g, layout = 'linear')
# edge_data <- layout$edges
# edge_data$mid_x <- (edge_data$x + edge_data$xend) / 2
# edge_data$mid_y <- (edge_data$y + edge_data$yend) / 2
# edge_data_df <- as.data.frame(edge_data)
# # Extract Freq from the graph object
# edge_data$Freq <- as.numeric(E(g)$Freq)
# 
# edge_data
# # Convert to edge list
# edge_data_df <- as.data.frame(get.edgelist(g, names = TRUE))
# colnames(edge_data_df) <- c("from", "to")
# 
# edge_data_df$Freq <- as.numeric(E(g)$Freq)
# edge_data_df$Freq
# # Add midpoint coordinates
# layout <- create_layout(g, layout = 'linear')
# edge_data_df$mid_x <- (layout$x[edge_data_df$from] + layout$x[edge_data_df$to]) / 2
# edge_data_df$mid_y <- (layout$y[edge_data_df$from] + layout$y[edge_data_df$to]) / 2
# 
# edge_data_df$mid_x 
# 
# # Get the edge list
...
# from_indices <- match(edge_list[, 1], V(g)$name)
# to_indices <- match(edge_list[, 2], V(g)$name)
# 
# # Create a data frame for the edge information
# edge_data_df <- data.frame(
#   from = from_indices,
#   to = to_indices,
#   Freq = E(g)$Freq
# )
...
# layout_info <- create_layout(g, layout = 'linear')
# 
# # Compute the midpoint coordinates
# edge_data_df$mid_x <- (layout_info$x[edge_data_df$from] + layout_info$x[edge_data_df$to]) / 2
# edge_data_df$mid_y <- (layout_info$y[edge_data_df$from] + layout_info$y[edge_data_df$to]) / 2
# 
# # Create the plot
# ggraph(g, layout = 'linear') +
#   geom_edge_arc(aes(alpha = Freq * 100 ), arrow = arrow(type = "closed"), show.legend = FALSE, colour = 'grey') +
#   geom_node_point(aes(size = size, color = color), show.legend = FALSE) +
#   geom_node_text(aes(label = paste0(scales::percent(size))), vjust = 1.5, hjust = 0.5) +
#   geom_text(data = edge_data_df, aes(x = mid_x, y = mid_y, label = scales::percent(Freq)), vjust = -2) +
...
# matrix graph
edges <- data.frame(
  from = rep(rownames(church_mat), each = ncol(church_mat)),
  to = rep(colnames(church_mat), times = nrow(church_mat)),
  label = as.vector(church_mat)
)


edges <- edges[edges$label != 0,]

ggraph(graph_church, layout = 'circle') +
  geom_edge_fan(aes(alpha = after_stat(index)), arrow = arrow(type = "closed"), show.legend = FALSE) +
#  geom_node_circle(aes(r = size / 100, fill = colour), colour = 'black') +
  geom_node_text(aes(label = name), vjust = -1.5, size = 5) +
  geom_text(data = edges, aes(x = (xend + x)/2, y = (yend + y)/2, label = label), vjust = 2, hjust = 0.5) +
  labs(title = "Transition Rates Between Religious Service Attendance States",
       subtitle = "Nodes represent states; edges represent transition rates") +
...
graph_church <- graph_from_adjacency_matrix(church_mat, mode = "directed", weighted = TRUE)
V(graph_church)$size <- c(85, 15) # Assign sizes to vertices based on percentage
V(graph_church)$colour <- ifelse(V(graph_church)$size == 85, "dodgerblue", "red") # Conditionally set colour
E(graph_church)$label <- c(0.997, 0.003, 0.016, 0.984) # Add edge labels as attributes
edges <- igraph::as_data_frame(igraph::get.edgelist(graph_church, names = TRUE))

edges <- igraph::as_data_frame(get.edgelist(graph_church, names = TRUE))
edges$label <- c(0.997, 0.003, 0.016, 0.984)



ggraph(graph_church, layout = 'circle') +
  geom_edge_fan(aes(alpha = after_stat(index)), show.legend = FALSE) +
  geom_node_circle(aes(r = size / 100), fill = 'steelblue') + # Map radius to vertex size
  geom_node_text(aes(label = name)) +# vjust = -1.5, size = 5) +
  labs(title = "Transition Rates Between Religious Service Attendance States",
       subtitle = "Nodes represent states; edges represent transition rates") +
...
graph_church <- graph_from_adjacency_matrix(church_mat, mode = "directed", weighted = TRUE)
 V(graph_church)$size <- c(85, 15) # Assign sizes to vertices based on percentage
V(graph_church)$colour <- ifelse(V(graph_church)$size == 85, "dodgerblue2", "red")
 E(graph_church)$label <- c(0.997, 0.003, 0.016, 0.984) # Add edge labels as attributes
edgelist <- igraph::get.edgelist(graph_church, names = TRUE)
edges <- data.frame(from = edgelist[, 1], to = edgelist[, 2])


layout <- create_layout(graph_church, layout = 'circle')

# Get coordinates of edge midpoints
edges <- data.frame(
  x = (layout$x[get.edgelist(graph_church)[, 1]] + layout$x[get.edgelist(graph_church)[, 2]]) / 2,
  y = (layout$y[get.edgelist(graph_church)[, 1]] + layout$y[get.edgelist(graph_church)[, 2]]) / 2,
  label = E(graph_church)$label
)

# Plot
ggraph(graph_church, layout = 'circle') +
  geom_edge_arc(aes(alpha = after_stat(index)), arrow = arrow(ends = "last")) +
  geom_node_point(aes(size = size, fill = colour), shape = 21) +
 # geom_node_circle(aes(r = size / 100), fill = 'white') + # Map radius to vertex size
  geom_node_text(aes(label = name))+# vjust = -1.5, size = 5) +
  geom_text(data = edges, aes(x = x, y = y, label = sprintf("%.3f", label)), vjust = 2, hjust = 0.5) +
  labs(title = "Transition Rates Between Religious Service Attendance States",
       subtitle = "Nodes represent states; edges represent transition rates") +
  theme_graph(foreground = 'white',
              fg_text_colour = 'white') +
...
church_mat <- matrix(
  c(0.997, 0.003, 0.016, 0.984),
  byrow = TRUE,
  nrow = 2
)

graph_church <- graph_from_adjacency_matrix(church_mat, mode = "directed", weighted = TRUE)
V(graph_church)$size <- c(85, 15)
V(graph_church)$colour <- ifelse(V(graph_church)$size == 85, "dodgerblue2", "red")
E(graph_church)$label <- c(0.997, 0.003, 0.016, 0.984)

layout <- create_layout(graph_church, layout = 'circle')

ggraph(graph_church, layout = 'circle') +
  geom_edge_arc(aes(alpha = after_stat(index)), arrow = arrow(ends = "last")) +
  geom_node_point(aes(size = size, colour = colour), shape = 21) +
  geom_node_circle(aes(r = size / 100), fill = 'white') +
  geom_node_text(aes(label = name)) +
  geom_edge_text(aes(label = sprintf("%.3f", label)), vjust = 2, hjust = 0.5) +
  labs(title = "Transition Rates Between Religious Service Attendance States",
       subtitle = "Nodes represent states; edges represent transition rates") +
  theme_graph(foreground = 'white',
              fg_text_colour = 'white') +
...
V(graph_church)$name <- c("State 1", "State 2") # Add names to vertices

ggraph(graph_church, layout = 'circle') +
  geom_edge_arc(aes(alpha = after_stat(index)), arrow = arrow(ends = "last")) +
  geom_node_point(aes(size = size, colour = colour), shape = 21) +
  geom_node_text(aes(label = name), repel = TRUE) + # Use name attribute for label
  geom_text(data = edges, aes(x = x, y = y, label = sprintf("%.3f", label)), vjust = 2, hjust = 0.5) +
  labs(title = "Transition Rates Between Religious Service Attendance States",
       subtitle = "Nodes represent states; edges represent transition rates") +
  theme_graph(foreground = 'white',
