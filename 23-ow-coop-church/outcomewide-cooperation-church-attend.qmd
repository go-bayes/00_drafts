...
baseline_vars_donate = mice_donate_long |> 
  dplyr::select(starts_with("t0"), -t0_religion_church_coarsen)|> colnames() # strange to include these -- as they  are income by other names
...
# # if trim is needed
match_donate_ebal_trim <- WeightIt::trim(match_donate_ebal, at = .997)
...
# if trim (if needed)
mice_warmth_ebal_trim <- WeightIt::trim(mice_warmth_ebal, at = .997)
...
baseline_vars_support = mice_support_long |> 
  dplyr::select(starts_with("t0"), -t0_religion_church_coarsen)|> colnames() # used for imputation -- fully co-linear
...
# trim if needed
match_support_ebal_trim <- WeightIt::trim(match_support_ebal, at = .997)
