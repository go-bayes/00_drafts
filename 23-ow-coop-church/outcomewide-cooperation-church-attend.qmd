...
push_mods <-  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23-ow-coop-church"
  )
...
# n_unique(dat$id)
# 
now <- dat |> filter(wave == 2018 & year_measured ==1)
n_unique(now$id)
...
# hist(dat$religion_church)
# mean(dat$religion_church, na.rm = TRUE)
# sd(dat$religion_church, na.rm = TRUE)
# # 
# # ## prop total pop
# now <- dat |> filter(wave == 2018 & year_measured == 1) |>
#   mutate(
...
#   filter(!is.na(bigger_doms))
# 
# 
# sum(dat_pr$religion_church ==0, na.rm=TRUE)/nrow(dat_pr)
# sum(dat_pr$religion_church ==0, na.rm=TRUE)/nrow(dat_pr)
# sum(dat_pr$religion_believe_god ==0, na.rm=TRUE)/nrow(dat_pr)
# 
# table1::table1(~as.factor(religious)| wave, data = now) #4.2%
# table1::table1(~as.factor(religion_church_binary)| wave, data = now) #4.2%
# table1::table1(~as.factor(religion_believe_god)| wave, data = now) #4.2%
# table1::table1(~as.factor(religion_believe_spirit)| wave, data = now) #4.2%
# 
# 
# 
# table1::table1(~as.factor(religion_believe_god)| wave, data = now) #4.2%
# table1::table1(~as.factor(believe_god_not_spirit)| wave, data = now) #4.2%
# table1::table1(~as.factor(believe_god_and_spirit)| wave, data = now) #40.2%
# table1::table1(~as.factor(not_believe_god_not_spirit)| wave, data = now) #26%
# table1::table1(~as.factor(not_believe_god_believe_spirit)| wave, data = now) #24%
# 
# 
# now_rel = now |> 
#   filter(religious==1)
# 
# table1::table1(~as.factor(religion_believe_god)| wave, data = now_rel) #84%
# table1::table1(~as.factor(believe_god_not_spirit)| wave, data = now_rel) #9%
# table1::table1(~as.factor(believe_god_and_spirit)| wave, data = now_rel) #75%
# table1::table1(~as.factor(not_believe_god_not_spirit)| wave, data = now_rel) #4%
# table1::table1(~as.factor(not_believe_god_believe_spirit)| wave, data = now_rel) #8.5%
# 
# 
# now_sec = now |> 
#   filter(religious==0)
# 
# table1::table1(~as.factor(religion_believe_god)| wave, data = now_sec) #23%
# 
# table1::table1(~as.factor(believe_god_not_spirit)| wave, data = now_sec) #9%
# table1::table1(~as.factor(believe_god_and_spirit)| wave, data = now_sec) #21%
# table1::table1(~as.factor(not_believe_god_not_spirit)| wave, data = now_sec) #57%
# table1::table1(~as.factor(not_believe_god_believe_spirit)| wave, data = now_sec) #34%
# 
# 
# 
# now_church = now |> 
#   filter(religion_church_binary == 1)
# 
# table1::table1(~as.factor(religion_believe_god)| wave, data = now_church) #93%
# table1::table1(~as.factor(believe_god_not_spirit)| wave, data = now_church) #88%
# table1::table1(~as.factor(believe_god_and_spirit)| wave, data = now_church) #81%
# table1::table1(~as.factor(not_believe_god_not_spirit)| wave, data = now_church) #1.8%
# table1::table1(~as.factor(religious)| wave, data = now_church) #4%
# table1::table1(~as.factor(religious)| wave, data = now_church) #4%
# 
# 
# 
...
# 
# now_atheist = now |> 
#   filter(not_believe_god_not_spirit==1)
# table1::table1(~as.factor(religion_church_binary)| wave, data = now_religious) #45%
# table1::table1(~as.factor(believe_god_not_spirit)| wave, data = now_religious) #9%
# table1::table1(~as.factor(believe_god_and_spirit)| wave, data = now_religious) #75%
# table1::table1(~as.factor(not_believe_god_not_spirit)| wave, data = now_religious) #92.4%%
# table1::table1(~as.factor(not_believe_god_believe_spirit)| wave, data = now_religious) #8.5%
# 
# 
# 
# 
# table1::table1(~as.factor(religion_church_binary)| bigger_doms, data = now, transpose = TRUE)
# table1::table1(~as.factor(religion_believe_god)| bigger_doms, data = now, transpose = TRUE)
# table1::table1(~as.factor(religion_believe_spirit)| bigger_doms, data = now, transpose = TRUE)
# 
# 
# table1::table1(~as.factor(believe_god_not_spirit)| bigger_doms, data = now,  transpose = TRUE) 
# table1::table1(~as.factor(believe_god_and_spirit)| bigger_doms, data = now ,transpose = TRUE) 
# table1::table1(~as.factor(not_believe_god_not_spirit)| bigger_doms, data = now, transpose = TRUE)
# table1::table1(~as.factor(not_believe_god_believe_spirit)| bigger_doms, data = now, transpose = TRUE) 
# table1::table1(~as.factor(religion_believe_god)| bigger_doms, data = now, transpose = TRUE) 
# table1::table1(~as.factor(religion_believe_spirit)| bigger_doms, data = now, transpose = TRUE) 
# 
# 
...
# # 
# ## prop religious
# dat_pr_rel <- dat |> filter( wave == 2018 & year_measured == 1 & religious ==1) |>  filter(!is.na(religion_church))
# sum(dat_pr_rel$religion_church >1, na.rm=TRUE)/nrow(dat_pr_rel)
# sum(dat_pr_rel$religion_church==0, na.rm=TRUE)/nrow(dat_pr_rel)
# sum(dat_pr_rel$religion_church>=8, na.rm=TRUE)/nrow(dat_pr_rel)
# 
# 
# # ## prop not religious
...
dat_long <- dat |>
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_church_binary), 1, 0)) |>   # creating an indicator for the first wave
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
  dplyr::mutate(k_19 =  ifelse(
    wave == 2019 &
      year_measured == 1 &
      !is.na(religion_church_binary),
    1,
...
## data for mice
exposure_vars = c("t1_religion_church_binary")
...
# Set exposure 
X <- "t1_religion_church_binary" # #levels( mice_donate_long$t1_religion_church_binary )
...
baseline_vars_donate = mice_donate_long |>
  ...
  #  -t0_religious_identification_level_z,
    -t0_religion_religious_z,
    -t0_religion_believe_god_z,
    -t0_religion_believe_spirit_z,
    -t0_pol_wing_z,
   -t0_bigger_doms)|> colnames() # strange to include these -- as they  are income by other names
...
match_donate_ebal <- match_mi_general(data = mice_donate_mids, 
                                ...
                                # focal = "zero",
                                 method = "ebal", 
                                 sample_weights = NULL)
here_save(match_donate_ebal, "match_donate_ebal")
match_donate_ebal <- here_read(( "match_donate_ebal"))
...
# graphs
plot(sum_ebal)
cobalt::bal.tab(match_donate_ebal, stats = c("m", "v"), binary = "std", thresholds = c(m = .1))
plot_love_no_trim <- love.plot(match_donate_ebal, binary = "std", thresholds = c(m = .1),  var.order = "unadjusted") # 
plot_love_no_trim

ggsave(
  plot_love_no_trim,
  path = here::here(here::here(push_mods, "plot_love_no_trim.png")),
  width = 16,
  height = 9,
  units = "in",
  filename = "gplot_love_no_trim.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
# # if trim is needed
match_donate_ebal_trim <- WeightIt::trim(match_donate_ebal, at = .999)
...
plot_love <- love.plot(match_donate_ebal_trim, binary = "std", thresholds = c(m = .1),  var.order = "unadjusted") # 
plot_love
# set dataframe
...
out <- msm::statetable.msm(religion_church_binary, id, data = dt_positivity_full)
#state_names <-c("zero", "one", "less_four", "four_up")
state_names <-c("church-no", "church-yes")
...
title = "ATE: +1 year causal effect from gain of religious service"
...
# ggsave(
#   group_plot_virtue,
#   path = here::here(here::here(push_mods, "group_plot_virtue_church_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtue-church-POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_virtue,
#   path = here::here(here::here("figs", "godin", "group_plot_virtue_church_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtue_church_POP",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rr-volunteers_church_population.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_ate_warmth_rd,
#   path = here::here(here::here(push_mods, "group_plot_ate_warmth_rd_church_POP")),
#   width = 8,
#   height = 4,
#   units = "in",
#   filename = "group_plot_ate_warmth_rd_church_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_ate_warmth_rd,
#   path = here::here(here::here("figs", "godin", "group_plot_ate_warmth_rd_church_POP")),
#   width = 8,
#   height = 4,
#   units = "in",
#   filename = "group_plot_ate_warmth_rd_church_POP.png",
...
#title = "ATE: +1 year causal effect of church gain"
...
# ggsave(
#   group_plot_support_rd,
#   path = here::here(here::here(push_mods, "group_plot_support_rd_church_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rd_church_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_support_rd,
#   path = here::here(here::here("figs", "godin", "group_plot_support_rd_church_POP.png")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rd_church_POP.png",
...
ggsave(
  group_plot_ate_support_rr,
  path = here::here(here::here(push_mods, "group_plot_ate_support_rr_church_POP")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "group_plot_ate_support_rr-church-POP.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
# ggsave(
#   group_plot_ate_support_rr,
#   path = here::here(here::here("figs","godin", "group_plot_ate_support_rr_church_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rr_church_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
dat_long_tX  <- dat_long %>%
  ...
  mutate(religion_church_coarsen_n = as.numeric(religion_church_coarsen) -
           1) |>
  mutate(
    religion_church_coarsen_n = as.numeric(religion_church_coarsen) - 1,
    religion_church_binary_n = as.numeric(religion_church_binary)
...
# set exposure variable, can be both the continuous and the coarsened, if needed
exposure_varX = c("religion_church_coarsen") # we could construct this after imputation. # "perfectionism_high" to be replace by "perfectionism_coarsen" - do the data wrangling after imputation.
...
## data for mice
exposure_varsX = c("t1_religion_church_coarsen")
...
levels( mice_donate_longX$t1_religion_church_coarsen)
# check
skim(mice_donate_longX)
...
#levels( mice_donate_longX$t1_religion_church_coarsen )
# Set exposure 
X <- "t1_religion_church_coarsen" #"t1_religion_church_coarsen" 
X
...
# graphs
cobalt::bal.tab(match_donate_ebalX, stats = c("m", "v"), binary = "std", thresholds = c(m = .1), which.treat = c("zero", "four_up"))
love.plot(match_donate_ebal_trimX, binary = "std", thresholds = c(m = .1),  var.order = "unadjusted", which.treat = c("zero", "four_up")) 
...
baseline_vars_warmthX = mice_warmth_longX |>  
  dplyr::select(
    starts_with("t0"),
    -t0_religion_church_coarsen,
  #  -t0_religion_prayer_binary_z,
...
# graphs
plot(sum_ebal_trimX)
bal.tab(mice_warmth_ebal_trimX,stats = c("m", "ks"), abs = TRUE, var.order = "unadjusted", which.treat = c("zero", "four_up"))
love.plot(mice_warmth_ebal_trimX, binary = "std", thresholds = c(m = .1),var.order = "unadjusted", which.treat = c("zero", "four_up")) # not great
...
# create baseline variables
baseline_vars_supportX = mice_support_longX |>   
  dplyr::select(
    starts_with("t0"),
    -t0_religion_identification_level_z,
    -t0_religion_believe_god_z,
...
love.plot(match_support_ebalX, binary = "std", thresholds = c(m = .1),var.order = "unadjusted", which.treat = c("zero", "four_up")) # not great
...
# trim if needed
match_support_ebal_trimX <- WeightIt::trim(match_support_ebalX, at = .995)
...
love.plot(match_support_ebal_trimX, binary = "std",  thresholds = c(m = .1),var.order = "unadjusted", which.treat = c("zero", "four_up")) # not great
...
out <- msm::statetable.msm(religion_church_coarsen, id, data = dt_positivity_fullX)
state_names <-c("zero", "one", "less_four", "four_up")
#state_names <-c("church-no", "church-yes")
...
titleX = "ATE: +1 year causal effect of +4 church gain (RELIGIOUS ONLY)"
...
# ggsave(
#   group_plot_virtueX,
#   path = here::here(here::here(push_mods, "group_plot_virtueX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtueX_church_REL.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_virtueX,
#   path = here::here(here::here('figs', "godin", "group_plot_virtueX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtueX_church_REL.png",
...
ggsave(
  gr_plot_virtue_rrX,
  path = here::here(here::here(push_mods, "gr_plot_virtue_rrX_church_REL")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "gr_plot_virtue_rrX_church_REL.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
# ggsave(
#   gr_plot_virtue_rrX,
#   path = here::here(here::here('figs', "godin",  "gr_plot_virtue_rrX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "gr_plot_virtue_rrX_church_REL.png",
...
#titleX = "ATE: +1 year causal effect of +4 church gain"
...
ggsave(
  group_plot_ate_warmth_rdX,
  path = here::here(here::here(push_mods, "group_plot_warmth_rdX_church_REL")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "group_plot_warmth_rdX_church_REL.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
# ggsave(
#   group_plot_ate_warmth_rdX,
#   path = here::here(here::here('figs', "godin",  "group_plot_warmth_rdX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_warmth_rdX_church_REL.png",
...
# ggsave(
#   group_plot_support_rdX,
#   path = here::here(here::here(push_mods, "group_plot_support_rdX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rdX_church_REL.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_support_rdX,
#   path = here::here(here::here('figs', "godin",  "group_plot_support_rdX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rdX_church_REL.png",
...
#| fig-cap: "Causal effects of church gain on revealed social supportX"
#| eval: true
#| include: true
#| echo: false
### Risk variables
...
ggsave(
  group_plot_ate_support_rrX,
  path = here::here(here::here(push_mods, "group_plot_ate_support_rrX_church_REL")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "group_plot_ate_support_rrX_church_REL.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
#  ggsave(
#   group_plot_ate_support_rrX,
#   path = here::here(here::here('figs', "godin", "group_plot_ate_support_rrX_church_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rrX_church_REL.png",
...
# # get unique identifiers of these individuals (replace `id` with your actual id variable)
# religious_ids <- dt_18_religious$id
levels(dat$bigger_doms)

#  select data of these individuals from the entire dataset
#  also create meaningful exposure variable
hist( dat$religion_church, breaks = 15 )
...
dat_long_tX1  <- dat %>%
 ...
 dplyr::filter((wave == 2018 & year_measured  == 1) |
  ...
  dplyr::mutate(k_19 =  ifelse(wave == 2019 &
                                 year_measured == 1 &
                                 !is.na(religion_church_round), #  Inclusion criteria
                               1,
                               0)) |>   # creating an indicator for the first wave; note that we allow people t
  dplyr::mutate(h_19 = mean(k_19, na.rm = TRUE)) |>  # Hack
  dplyr::filter(h_18 > 0) |>  # hack to enable repeat of baseline
...
# 33198
# check n again
n_unique(dat_long_tX1$id) #32964
...
# set exposure variable, can be both the continuous and the coarsened, if needed
exposure_varX1 = c("religion_church_round","not_censored") # 
...
df_clean_donate <- df_wide_censored_donate %>%
 ...
 # select(-t2_na_flag) %>%
  ...
  dplyr::mutate(across(where(is.numeric) &
                        !t0_not_censored & !t1_not_censored & !t0_sample_weights & !t1_religion_church_round & !t2_charity_donate & 
                         !t2_gratitude & !t2_hours_charity, ~ scale(.x), .names = "{col}_z"))|>
  select(where(is.factor), t0_not_censored, t1_not_censored, t0_sample_weights, t1_religion_church_round, t2_gratitude,  t2_charity_donate,t2_hours_charity, ends_with("_z")) |>
  relocate(starts_with("t0_"), .before = starts_with("t1_"))  %>%
  relocate(starts_with("t2_"), .after = starts_with("t1_"))  %>%
  relocate("t0_not_censored", .before = starts_with("t1_"))  %>%
  relocate("t1_not_censored", .before = starts_with("t2_")) |> 
...
n_unique(df_clean_donate$id)#32964
...
# check: 
table( df_clean_donate$t1_religion_church_round )
...
m_t2_charity_donate <- here_read("m_t2_charity_donate")
...
contrast_donate_full <- lmtp_contrast(m_t2_charity_donate,ref = m_t2_charity_donate_null, type = "additive")
...
min_annual_donate_gained = nz_adult_population * 430

(min_annual_donate_gained + hours_amount) /nz_annual_budget

min_annual_donate_gained + hours_amount
...
dat_2022 <- dat |> 
  ...
  select( religion_perceive_religious_discrim, religion_identification_level, bigger_doms) |> 
  drop_na()

nrow(dat_2022)
 mod_1 <-  lm(religion_perceive_religious_discrim ~ bs(religion_identification_level), dat_2022) 


plot_rel_id <-plot(ggeffects::ggpredict(mod_1, effects = "religion_identification_level"), add.data = TRUE, dot.alpha =.05)
plot_rel_id
 
plot_rel_id <- ggeffects::ggpredict(mod_1, effects = "religion_identification_level")
# 
# # Get the predicted values
# predictions <- ggeffects::ggpredict(mod_1, effects = "religion_identification_level")
# 
# # Extract the data
# data <- as.data.frame(predictions)
# # Plot using ggplot
# plot_rel_pred_rel_id <- ggplot(data, aes(x = religion_identification_level.x, y = religion_identification_level.predicted)) +
#   geom_line() +
#   geom_ribbon(aes(ymin = religion_identification_level.conf.low, ymax = religion_identification_level.conf.high), alpha = 0.2) +
#   labs(
#     x = "Strength of Religious Identification",
#     y = "Perceived Religious Discrimination",
#     title = "Predicted Values of Perceived Prejudice by Strength of Religious Identification NZAVS, year = 2021-22, N = 32723"
#   ) + scale_y_continuous(limits = c(1,7))+
#   theme_classic()

# Display the plot
plot_rel_pred_rel_id

pl <- plot_rel_id$religion_identification_level + theme_classic() +    labs(
    x = "Strength of Religious Identification (1-7)",
...
ggsave(
  pl,
  path = here::here(here::here(push_mods, "use_plot_rel_id.jpg")),
  width = 6,
  height =8,
  units = "in",
  filename = "use_plot_rel_id.jpg",
  device = 'jpg',
  limitsize = FALSE,
  dpi = 600
)

mod_2 <-  glm(religion_perceive_religious_discrim ~ bigger_doms, dat_2022) 
mod_2

plot_rel_bigger_doms <- plot(ggeffects::ggpredict(mod_2), add.data = TRUE, dot.alpha =.002)[1] 
plot_rel_bigger_doms <- plot_rel_bigger_doms$bigger_doms

plot_rel_bigger_doms



ggsave(
  plot_rel_bigger_doms,
  path = here::here(push_mods, "plot_rel_bigger_doms.jpg"),
  width = 16,
  height = 9,
  units = "in",
  filename = "plot_rel_bigger_doms.jpg",
  device = 'jpg',
  limitsize = FALSE,
  dpi = 600
)
