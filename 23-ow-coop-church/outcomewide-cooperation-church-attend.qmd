...
# n_unique(dat$id)
# 
# now <- dat |> filter(wave == 2021 & year_measured ==1) 
# n_unique(now$id)
...
# 
# ## prop total pop
# dat_pr <- dat |> filter( wave == 2018 & year_measured == 1) |>  filter(!is.na(religion_church))
...
dat_long <- dat |>
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_church_binary) & !is.na(bigger_doms), 1, 0)) |>   # creating an indicator for the first wave
...
table( dat_long$religion_church_binary )
...
dat_long_tX1  <- dat %>%
 ...
 dplyr::filter((wave == 2018 & year_measured  == 1) |
  ...
  group_by(id) |>
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_church_round) & !is.na(eth_cat), 1, 0)) |>   # creating an indicator for the first wave. Inclusion criteria
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
...
proportion = sum(df_clean_donate$t1_religion_church_round >= 8) / nrow(df_clean_donate)
proportion
