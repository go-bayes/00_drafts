---
title: "Causal effects of church attendance on cooperation"
...
dat_long <- dat |>
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_church_binary2) & !is.na(bigger_doms), 1, 0)) |>   # creating an indicator for the first wave
...
table( dat_long$religion_church2 )



dat_1 <- dat |> 
  filter(wave == 2010 | wave == 2019) 
  

table1::table1(~ religion_church| wave, data=dat_1)
...
#  filter(id %in% religious_ids) %>%   only if subsetting religious ids 
  ...
  mutate(
    ...
    religion_church_binary2_n = as.numeric(religion_church_binary2)
  ) |>
  mutate(
    religion_church_binary2 = as.factor(religion_church_binary2),
...
exposure_var = c("religion_church_binary2") # we could construct this after imputation. # "perfectionism_high" to be replace by "perfectionism_coarsen" - do the data wrangling after imputation.
...
## data for mice
exposure_vars = c("t1_religion_church_binary2")
...
# Set exposure 
X <- "t1_religion_church_binary2" #"t1_religion_church_coarsen" #levels( mice_donate_long$t1_religion_church_coarsen )
...
# baseline_vars_donate_exposure = mice_donate_long |>
#   dplyr::select(
#     starts_with("t0"),
#     -t0_religion_church_coarsen,
#     -t0_religion_believe_god_z,
...
baseline_vars_warmth
# 
# baseline_vars_exposure = mice_donate_long |>
#   dplyr::select(
#     starts_with("t0"),
#     -t0_religion_church_coarsen,
#     -t0_religion_believe_god_z,
...
out <- msm::statetable.msm(religion_church_binary2, id, data = dt_positivity_full)
...
title = "ATE: +1 year causal effect from gain of church"
...
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtue_CHURCH_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_support_rd,
#   path = here::here(here::here("figs", "godin", "group_plot_support_rd_CHURCH_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rd_CHURCH_POP.png",
...
# ggsave(
#   group_plot_ate_support_rr,
#   path = here::here(here::here(push_mods, "group_plot_ate_support_rr-CHURCH-POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
...
# ggsave(
#   group_plot_ate_support_rr,
#   path = here::here(here::here("figs","godin", "group_plot_ate_support_rr-CHURCH-POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rr-CHURCH-POP.png",
...
"nzsei13",
"have_siblings",
# added: needed because we are dealing with family giving/receiving
"born_nz",
# added
"hlth_disability",
# added
...
"children_num",
"household_inc_log",
# new from previous study. needed because we are dealing with family giving/receiving
"hours_children_log",
# new
"hours_work_log",
# new
...
baseline_vars_warmthX = mice_warmth_longX |> 
  dplyr::select(starts_with("t0"), -t0_bigger_doms)|> colnames() # strange to include these -- as they  are income by other names
...
str(dat_long_tX$religion_church_binary2)
...
# family money rr
mod_support_family_money_binary_rrX  <- double_robust(
  df = dt_supportX,  # note change
  Y = "t2_family_money_binary",
  X = X,
  baseline_vars = 1,#baseline_vars_supportX,  ## always check
  treat_1 = treat_1,
  treat_0 = treat_0,
  estimand = estimand,
  scale = "RR",
...
mod_support_family_money_binary_rrX
saveRDS(mod_support_family_money_binary_rrX, here::here(push_mods, "mod_support_family_money_binary_rrX"))


# friends money rr
mod_support_friends_money_binary_rrX  <- double_robust(
  df = dt_supportX,  # note change
  Y = "t2_friends_money_binary",
  X = X,
  baseline_vars = 1, #baseline_vars_supportX,  ## always check
...
mod_support_friends_money_binary_rrX
saveRDS(mod_support_friends_money_binary_rrX, here::here(push_mods, "mod_support_friends_money_binary_rrX"))

# comunity money rr
mod_support_community_money_binary_rrX  <- double_robust(
  df = dt_supportX,  # note change
  Y = "t2_community_money_binary",
  X = X,
...
mod_support_community_money_binary_rrX
saveRDS(mod_support_community_money_binary_rrX, here::here(push_mods, "mod_support_community_money_binary_rrX"))
#mod_support_community_money_binary_rr<- here_read("mod_support_community_money_binary_rr")
### support continuous 
# mod_donate_support <- double_robust(
#   df = dt_supportX,
...
dat_long_tX1  <- dat %>%
 # filter(id %in% religious_ids) %>%
  ...
  mutate(religion_religious = as.numeric(religion_religious) - 1) |>
  mutate(
    religion_church_round = round( ifelse(religion_church2 > 4, 5, religion_church2), 0) )|> 
  mutate(
    eth_cat = as.integer(eth_cat),
    urban = as.numeric(urban),
    education_level_coarsen = as.integer(education_level_coarsen)
 ...
 dplyr::filter((wave == 2018 & year_measured  == 1) |
  ...
  group_by(id) |>
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_church2), 1, 0)) |>   # creating an indicator for the first wave. Inclusion criteria
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
  dplyr::mutate(k_19 =  ifelse(wave == 2019 &
                                 year_measured == 1 &
                                 !is.na(religion_church2), #  Inclusion criteria
                               1,
                               0)) |>   # creating an indicator for the first wave; note that we allow people t
  dplyr::mutate(h_19 = mean(k_19, na.rm = TRUE)) |>  # Hack
  dplyr::filter(h_18 > 0) |>  # hack to enable repeat of baseline
...
prep_warmthX1 <- margot_wide(
dat_long_tX1,
baseline_vars = baseline_varsX1,
exposure_var = exposure_varX1,
outcome_vars = outcome_vars_warmthX1
)
...
prep_supportX1 <- margot_wide(
dat_long_tX1,
baseline_vars = baseline_varsX1,
exposure_var = exposure_varX1,
outcome_vars = outcome_vars_supportX1
)
...
## data for mice
# exposure_varsX1 = c("t1_religion_church_round")
# 
# here_save(exposure_varsX1 , "exposure_varsX1")
...
df_wide_censored_support<- 
  prep_supportX1 |> 
  relocate("t0_not_censored", .before = starts_with("t1_"))  %>%
  relocate("t1_not_censored", .before = starts_with("t2_"))


head( df_wide_censored_donate$ t2_charity_donate ) 
 ...
 # select(-t2_na_flag) %>%
  ...
  dplyr::mutate(across(where(is.numeric) &
                        !t0_not_censored & !t1_not_censored & !t0_sample_weights & !t1_religion_church_round & !t2_charity_donate & 
                         !t2_gratitude, ~ scale(.x), .names = "{col}_z"))|>
  select(where(is.factor), t0_not_censored, t1_not_censored, t0_sample_weights, t1_religion_church_round, t2_gratitude,  t2_charity_donate, ends_with("_z")) |>
  relocate(starts_with("t0_"), .before = starts_with("t1_"))  %>%
  relocate(starts_with("t2_"), .after = starts_with("t1_"))  %>%
  relocate("t0_not_censored", .before = starts_with("t1_"))  %>%
  relocate("t1_not_censored", .before = starts_with("t2_")) |> 
...
n_unique(df_clean_donate$id)#10402

df_clean_warmth <- df_wide_censored_warmth %>%
  mutate(t2_na_flag = rowSums(is.na(select(., starts_with("t2_")))) > 0) %>%
  mutate(t1_not_censored = ifelse(t2_na_flag, 0, t1_not_censored)) %>%
 # select(-t2_na_flag) %>%
  filter(!rowSums(is.na(select(., starts_with("t0_"))))) |> 
  dplyr::mutate(across(where(is.numeric) &
                        !t0_not_censored & !t1_not_censored & !t0_sample_weights & !t1_religion_church_round, ~ scale(.x), .names = "{col}_z")) |>
  select(where(is.factor), t0_not_censored, t1_not_censored, t0_sample_weights, t1_religion_church_round, ends_with("_z")) |>
  relocate(starts_with("t0_"), .before = starts_with("t1_"))  %>%
  relocate(starts_with("t2_"), .after = starts_with("t1_"))  %>%
  relocate("t0_not_censored", .before = starts_with("t1_"))  %>%
  relocate("t1_not_censored", .before = starts_with("t2_")) |> 
...
colnames(df_clean_warmth)
n_unique(df_clean_warmth$id)#10402
table(df_clean_warmth$t1_not_censored)


df_clean_support <- df_wide_censored_support %>%
  mutate(t2_na_flag = rowSums(is.na(select(., starts_with("t2_")))) > 0) %>%
  mutate(t1_not_censored = ifelse(t2_na_flag, 0, t1_not_censored)) %>%
 # select(-t2_na_flag) %>%
  filter(!rowSums(is.na(select(., starts_with("t0_"))))) |> 
...
colnames(df_clean_support)
n_unique(df_clean_support$id)#10402


table(df_clean_support$t1_not_censored)

# Continue on with the other steps


head(df_clean_support)
...
# check: 
df_clean_donate$t1_religion_church_round
...
min(df_clean_donate$t1_religion_church_round, na.rm=TRUE)
...
# f <- function(data, trt){
#   ifelse( data[[trt]] < 4, 4,  data[[trt]] + 1)
# }
# 



#f <- function(data, trt) data[[trt]] + 1 # add one church to all church goers
...
m_t2_hours_charity_z <- lmtp_tmle(data = df_clean_donate, 
                                   trt = A, 
                                 ...
                                 #  id = "id",
                                   ...
                                   learners_trt =  "SL.earth",
                                   learners_outcome =  "SL.earth")


here_save(m_t2_hours_charity_z, "m_t2_hours_charity_z")
m_t2_hours_charity_z
...
interv = 1181.9471 * population_not_attending
 
# 5,542,555,713
base = 1100.29 * 5123000

 base - interv

population_not_attending <- 5123000 - (5123000 *proportion)

mean ( df_clean_donate$t2_charity_donate, na.rm = TRUE)



m_t2_honesty_humility_z <- lmtp_tmle(
  data = df_clean_donate,
  trt = A,
  baseline = names_base_donate,
  outcome = "t2_honesty_humility_z",
  cens = C,
  shift = f,
  mtp = TRUE,
  folds = 10,
...
here_save(m_t2_honesty_humility_z, "m_t2_honesty_humility_z")
m_t2_honesty_humility_z


m_t2_gratitude <- lmtp_tmle(
  data = df_clean_donate,
  trt = A,
  baseline = names_base_donate,
  outcome = "t2_gratitude",
  cens = C,
  shift = f,
  mtp = TRUE,
  folds = 10,
...
here_save(m_t2_gratitude, "m_t2_gratitude")
m_t2_gratitude
...
mean(df_clean_donate$t2_gratitude, na.rm = TRUE)
5.959 -5.944314
# 5,542,555,713
base = 1100.29 * 5123000
...
m_t2_vengeful_rumin_z <- lmtp_tmle(
  data = df_clean_donate,
  trt = A,
  baseline = names_base_donate,
...
here_save(m_t2_vengeful_rumin_z, "m_t2_vengeful_rumin_z")
m_t2_vengeful_rumin_z

m_t2_modesty_z <- lmtp_tmle(
  data = df_clean_donate,
  trt = A,
  baseline = names_base_donate,
...
here_save(m_t2_modesty_z, "m_t2_modesty_z")
m_t2_modesty_z
