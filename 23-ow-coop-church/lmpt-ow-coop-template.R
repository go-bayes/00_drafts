# lmpt-ow-coop-template.R
# outcome-wide- coopearation- nalysis-template
# sept 25, 2023
...
# shift function for church attendance 
...
# uncomment if a continuous exposure
#dt_19 <- dat |> 
...
# kessler 6 ---------------------------------------------------------------
# uncomment if you need to reduce dimensionality of constructs
# 
# 
# 
...
table (is.na( dt_check_exposure$religion_church_round) )
...
# save
here_save(t2_hours_charity, "t2_hours_charity")



t2_hours_charity_null <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
  outcome = "t2_hours_charity",
...
# view
t2_hours_charity_null
here_save(t2_hours_charity_null, "t2_hours_charity_null")
...
# under null
t2_charity_donate_null <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
  outcome = "t2_charity_donate_z",
...
here_save(t2_charity_donate_null, "t2_charity_donate_null")
t2_charity_donate_null
...
t2_community_time_binary <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
t2_community_time_binary
here_save(t2_community_time_binary, "t2_community_time_binary")
...
here_save(t2_community_time_binary_null, "t2_community_time_binary_null")
...
t2_friends_time_binary <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
t2_friends_time_binary
here_save(t2_friends_time_binary, "t2_friends_time_binary")
...
t2_family_time_binary <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
t2_family_time_binary
here_save(t2_family_time_binary, "t2_family_time_binary")
...
t2_warm_asians_z
here_save(t2_warm_asians_z, "t2_warm_asians_z")
...
t2_warm_asians_z_null


t2_warm_chinese_z <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
t2_warm_chinese_z_null


t2_warm_immigrants_z <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
here_save(t2_warm_immigrants_z, "t2_warm_immigrants_z")
...
t2_warm_indians_z
here_save(t2_warm_indians_z, "t2_warm_indians_z")
...
t2_warm_elderly_z
here_save(t2_warm_elderly_z, "t2_warm_elderly_z")
...
t2_warm_maori_z
here_save(t2_warm_maori_z, "t2_warm_maori_z")
...
t2_warm_maori_z_null


t2_warm_mental_illness_z <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
here_save(t2_warm_mental_illness_z, "t2_warm_mental_illness_z")
...
t2_warm_muslims_z
here_save(t2_warm_muslims_z, "t2_warm_muslims_z")
...
t2_warm_nz_euro_z
here_save(t2_warm_nz_euro_z, "t2_warm_nz_euro_z")
...
here_save(t2_warm_overweight_z, "t2_warm_overweight_z")
...
t2_warm_overweight_z_null




t2_warm_pacific_z <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
t2_warm_pacific_z
here_save(t2_warm_pacific_z, "t2_warm_pacific_z")
...
t2_warm_refugees_z
here_save(t2_warm_refugees_z, "t2_warm_refugees_z")
...
t2_warm_refugees_z_null
here_save(t2_warm_refugees_z_null, "t2_warm_refugees_z_null")




t2_religion_perceive_religious_discrim_z <- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
...
t2_religion_perceive_religious_discrim_z
here_save(t2_religion_perceive_religious_discrim_z, 
		"t2_religion_perceive_religious_discrim_z")

t2_religion_perceive_religious_discrim_z_null<- lmtp_tmle(
  data = df_clean,
  trt = A,
  baseline = names_base,
  outcome = "t2_religion_perceive_religious_discrim_z",
...
t2_religion_perceive_religious_discrim_z_null
here_save(t2_religion_perceive_religious_discrim_z_null, "t2_religion_perceive_religious_discrim_z_null")
...
# volunteers
t2_hours_charity <- here_read("t2_hours_charity")
t2_hours_charity_null <-
  here_read("t2_hours_charity_null")


contrast_t2_hours_charity <-
  lmtp_contrast(t2_hours_charity,
                ref = t2_hours_charity_null,
                type = "additive")


tab_contrast_t2_hours_charity <-
  margot_tab_lmtp(tab_contrast_t2_hours_charity,
                  scale = "RD",
                  new_name = "Volunteers")
...
out_tab_contrast_t2_hours_charity <-
  lmtp_evalue_tab(tab_contrast_t2_hours_charity,
                  scale = c("RD"))
...
t2_charity_donate_z_null <-
  here_read("t2_charity_donate_z_null")


contrast_t2_charity_donate_z <-
  lmtp_contrast(t2_charity_donate_z,
                ref = t2_charity_donate_z_null,
                type = "additive")


tab_contrast_t2_charity_donate_z <-
  margot_tab_lmtp(contrast_t2_charity_donate_z,
                  scale = "RD",
                  new_name = "Donates")
...
out_tab_contrast_t2_charity_donate_z <-
  lmtp_evalue_tab(tab_contrast_t2_charity_donate_z,
                  scale = c("RD"))
...
t2_community_time_binary <- here_read("t2_community_time_binary")
t2_community_time_binary_null <-
  here_read("t2_community_time_binary_null")


contrast_t2_community_time_binary <-
  lmtp_contrast(t2_community_time_binary,
                ref = t2_community_time_binary_null,
                type = "rr")


tab_contrast_t2_community_time_binary <-
  margot_tab_lmtp(contrast_t2_community_time_binary,
                  scale = "RR",
                  new_name = "Community offers time")

tab_contrast_t2_community_time_binary


out_tab_contrast_t2_community_time_binary <-
  lmtp_evalue_tab(tab_contrast_t2_community_time_binary,
                  scale = c("RR"))
...
t2_friends_time_binary <- here_read("t2_friends_time_binary")
t2_friends_time_binary_null <-
  here_read("t2_friends_time_binary_null")


contrast_t2_friends_time_binary <-
  lmtp_contrast(t2_friends_time_binary,
                ref = t2_friends_time_binary_null,
                type = "rr")


tab_contrast_t2_friends_time_binary <-
  margot_tab_lmtp(contrast_t2_friends_time_binary,
                  scale = "RR",
                  new_name = "Friends offers time")

tab_contrast_t2_friends_time_binary


out_tab_contrast_t2_friends_time_binary <-
  lmtp_evalue_tab(tab_contrast_t2_friends_time_binary,
                  scale = c("RR"))
...
t2_family_time_binary <- here_read("t2_family_time_binary")
t2_family_time_binary_null <- here_read("t2_family_time_binary_null")


contrast_t2_family_time_binary <-
  lmtp_contrast(t2_family_time_binary,
                ref = t2_family_time_binary_null,
                type = "rr")


tab_contrast_t2_family_time_binary <-
  margot_tab_lmtp(contrast_t2_family_time_binary,
                  scale = "RR",
                  new_name = "Family offers time")
