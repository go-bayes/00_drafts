...
dt_19 <- dat_long |> 
  filter(wave == "2019") |> 
  mutate(perfectionism_quartile_number = as.numeric(perfectionism_quartile))

dt_18_19 <- dat_long |> 
  filter(wave == "2018", wave == "2019") |> 
  mutate(perfectionism_quartile_number = as.numeric(perfectionism_quartile)) |> 
  mutate(perfectionism_quartile_number = as.numeric(as.character(perfectionism_quartile)))



table(dt_18_19$perfectionism_quartile_number)
...
# Create labels based on the cut points in the desired format
cut_labels <- paste0(quantile_breaks[-length(quantile_breaks)], "to", quantile_breaks[-1])

cut_labels 

# Create the ordered factor variable with the new labels
# set order to FALSE only for PROP score models
mc$t0_perfectionism_quartile <- cut(mc$t0_perfectionism,
                                        breaks = quantile_breaks,
                                        labels = cut_labels,
                                        ordered_result = TRUE,
