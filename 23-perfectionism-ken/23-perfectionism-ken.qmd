...
dt_match_trim_health <- WeightIt::trim(dt_match_health, at = .95)


sum <- summary(dt_match_trim_health)
sum
plot(sum)
bal.tab(dt_match)
dt_match
...
# settings 
dt_match_health_use = dt_match_trim_health # matched dataset from propensity scores.  If no propensity score model make ml_match = ml
...
# weighted.models <- with(dt_match_health_use,
...
weighted.models <- with(dt_match_health_use,
                        glm(t2_alcohol_frequency_z ~ t1_perfectionism_high, family = gaussian))


summary(weighted.models)

baseline_vars_health

test <- causal_contrast( df = dt_match_health_use,  # note change
  Y = "t2_alcohol_frequency_z",
  X = "t1_perfectionism_high",
...
mod_alcohol_frequency <- causal_contrast(
  df = dt_match_health_use,
