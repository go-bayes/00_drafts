...
# num_quantiles <- 4 
# 
# quantile_breaks <- quantile(dat$perfectionism, probs = seq(0, 1, 1/num_quantiles))
# dat$perfectionism_quantile <- cut(dat$perfectionism, breaks = quantile_breaks, include.lowest = TRUE, labels = FALSE)
# 
# dat_q <- dat |> mutate(perfectionism_quartile = ifelse(is.na(perfectionism), NA, ntile(perfectionism, 4)))
# 
# hist( dat$perfectionism_quartile, breaks = 100) 
# 
# hist( dat$perfectionism_quartile, breaks = 100) 
# 
# test_dat <- dat |> filter(!is.na(perfectionism))
# head(test_dat[, c("perfectionism", "perfectionism_quartile")])
# 
# # Calculate the quantile breaks (cut points)
# quantile_breaks <- quantile(dat$perfectionism, probs = seq(0, 1, 1/4), na.rm = TRUE)
# 
# # Print the quantile breaks
...
table( 7-  dat$self_control02r  )
table( dat$self_control02r  )
dat$sexual_satisfaction

dat_long <- dat |> 
  arrange(id, wave) |> 
    # select variables 
  mutate(religion_church_binary = as.factor(ifelse(religion_church > 0, 1, 0))) |> 
  mutate(power_self_nocontrol = powerdependence1,
         power_others_control = powerdependence1,
         self_control_have_lots = self_control01,
         self_control_wish_more = 7 -self_control02r, # original reverse scored
             eth_cat = as.factor(eth_cat)) |> 
  rename(religion_identification_level = religious_identification_level) |> 
...
table(dat_long$perfectionism_quartile)

table(is.na( dat_long$male))
# 
# ## target trial 
# dat_long <- dat_long |>
#   dplyr::filter((wave == 2018 & year_measured  == 1) |
...
selected_health <- prepare_data(
  ...
  exposure_var = c("perfectionism", "perfectionism_qaurtile"),
  outcome_vars = c(
    "alcohol_frequency",
    "alcohol_intensity",
    "hlth_bmi",
...
selected_embodied_wellbeing <- prepare_data(
  data = dat_long,
  baseline_vars = c(
    "edu",
    "male",
...
selected_practical_wellbeing <- prepare_data(
  data = dat_long,
  baseline_vars = c(
    "edu",
    "male",
  ...
  exclude_vars = c("year_measured","wave")
)

str(selected_practical_wellbeing)



# higher order.. gratitude could go here. 


selected_reflective_wellbeing <- prepare_data(
  data = dat_long,
  baseline_vars = c(
   "edu",
    "male",
...
create_wide_data <- function(dat_long, baseline_vars, exposure_var, outcome_vars, exclude_vars = c()) {
  ...
  # Define a custom function to filter columns based on your conditions
...
test <- create_wide_data_generic(dat_long, 
  baseline_vars = c(
   "edu",
    "male",
    "eth_cat",
...
# Calculate the quantile breaks (cut points)
quantile_breaks <- quantile(dat_long$perfectionism, probs = seq(0, 1, 1/4), na.rm = TRUE)
...
dat_long$perfectionism_quartile <- cut(dat_long$perfectionism,
...
# Print the resulting data frame
print(dat_long$perfectionism_quartile)
...
dt_match <- match_mi(ml_18, X = X, baselinevars = baselinevars, estimand = "ATE", method = "ebal")
...
rm(mod_test_only_for_code)

tab_ate(mod_test_only_for_code, new_name = "work", scale = "RD")
# rownames(mod_test_only_for_code)<- "RD"
# mod_test_only_for_code


test_you <- gcomp_sim(
  df = dt_match,  # note change
  Y = "t3_religion_secular",
  X = "t1_religious_change",
  # X = "t1_test",
  baseline_vars = baseline_vars,
...
mod_test_only_for_code2 <- cauxsal_contrast(
  df = dt_match,
  Y = "t3_religion_secular",
  X = "t1_religious_change_ignore_test_z",
  baseline_vars = baseline_vars,
