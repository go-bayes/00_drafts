...
Let us assume that $A$ exists in two states: $A \in \{0,1\}$. If $A$ is $0$ we denote the potential outcome as $Y(0)$. If $A$ is set to 1, we denote the potential outcome as $Y(1)$. We call $Y(1), Y(0)$ 'potential outcomes' because, until realised, the outcomes are counterfactual. 
...
Thuse where $\delta_i$ is the quantity of interest,
...
Do not adopting an indiscriminate approach to confounding control, what Richard McElreath aptly calls the "causal salad"[@mcelreath2020]. Even if we can accurately measure the occurrence of our variables in time, conditioning on a pre-exposure variable may induce bias. Each problem must be considered in light of its features, by the best-lights of subject-matter experts.

This solution to the problem of conditioning on a pre-exposure collider is given in @fig-m-bias-solution. 

```{tikz}
#| label: fig-m-bias-solution
#| fig-cap: "M-bias is avoided when by not conditioning on pre-exposure variable L. Doing so reveals all open backdoor paths are blocked. They are blocked because L is a collider of the unmeasured variables U1 and U2. If we do not condition on L, the path between U1 and U2 remain closed."
#| out-width: 80%
#| echo: false
...
Certainly, let's implement the suggestions to improve the clarity and accuracy of your example:
...
Let $U$ represent ancestral language families, which influence the development of unique social rituals $A$. For instance, isolated language families may develop distinct cultural practices. We assume $U$ is unmeasured. Now, let $S$ denote the sample of available cultures. Here, we consider that $S$ is affected by language families in such a way that anthropologists may have a preference for studying cultures from certain ancestral language families. This preference leads to a sample that is not representative of all cultures. Suppose there is no direct causal link between ancestral language family $U$ and technological advancement $Y$. However, we posit that technologically advanced societies ($Y$) are more likely to be documented due to better accessibility and more comprehensive documentation. This documentation bias affects the sample $S$. We also assume there is no causal association between $A$ and $Y$.
...
We cannot address this form of selection bias through confounding control. Here, our causal diagram is useful because it tells use we need to stop, and consider how to recover unbaised measurements of Y. 

#### Case when conditioning on a descendant reduces bias

Consider a scenario where adjusting for a post-treatment descendant variable, denoted as $L^\prime$, can help mitigate bias. Imagine an unmeasured confounder $U$ that influences $A$, $Y$, and $L^\prime$, with the effect on $L^\prime$ occurring after both $A$ and $Y$. In this context, adjusting for $L^\prime$ could potentially lessen the confounding caused by the unmeasured confounder $U$. This approach aligns with the modified disjunctive cause criterion for confounding control, which suggests including as a covariate any proxy for an unmeasured variable that is a common cause of both the exposure and the outcome [@vanderweele2019]. As illustrated in @fig-dag-descendent-solution-2, although $L^\prime$ occurs after the exposure and possibly even after the outcome, conditioning on it can reduce confounding.

How does this work in practice? For example, consider a genetic factor that influences both the exposure and the outcome early in life but is expressed later. Adjusting for this later-life expression of the genetic factor can help control for the unmeasured confounding due to its early influence on both $A$ and $Y$. Here, conditioning on $L'$ is a sensible strategy and provides an instance of post-outcome confounding control. This scenario is depicted in @fig-dag-descendent-solution-2.
...
The prospect that we may use descendants for confounding control reveals that even if for a causal diagram, "timing is everything," when it comes to the analysis of a problem, **structure is everything**. The chronologically hygienic graph reveals scope for conditioning strategies on confounders measures after the exposure or outcome. It brings home the point that we should think of the concept of a confounder as meaningful only in relation to the adjustment set in which it forms a part.

We are now in position to understand why I advocate using VanderWeele's modified disjunctive cause criteria for selecting this confounder set because it will remove all instrumental variables unless they are they are descendants of unmeasured common confounders of the exposure and outcome. Neither U1 nor U2 satisfy this property.

Practically speaking, determining which variables belong in the confounder set can be challenging. We can only be guided by the best lights of specialists. However, science is the practice of revising expert opinion, lifting the standard of those lights. Therefore it is critical to do our best at confounding control, and then perform sensitivity analyses.

### Case 1: Causal Interaction and Causal Effect Modification

Causal investigations often explore how effects vary across sub-populations or how the joint effect of two interventions differs from their individual effects or no intervention at all. This inquiry into causal interaction is scientifically significant.

#### Graphical Depiction of Causal Interactions
When depicting causal interactions on graphs, it's important to focus on their primary purpose: examining confounding. These diagrams are not designed to represent non-linear relationships or interactions in detail. Instead, they should identify conditional independencies (represented as $\coprod$) crucial for unbiased causal inference. Avoiding unnecessary complexities in these diagrams, such as trying to visually represent additive or multiplicative interactions, keeps the focus on identifying potential sources of bias. The inclusion of nodes and paths should be strictly limited to what is necessary for assessing the specified causal contrasts.
...
For instance, consider the effect of beliefs in Big Gods (exposure $A$) on social complexity (outcome $Y$), potentially influenced by a culture's monumental architecture (exposure $B$). To assess the individual and combined effects of $A$ and $B$, we look for evidence of causal interaction on the difference scale. Evidence would be indicated if the following inequality holds:
...
If this condition is satisfied, it suggests the effect of exposure \( A \) on \( Y \) varies with the level of \( B \), or vice versa, indicating interaction. A positive value suggests additive interaction, a negative value suggests sub-additive interaction, and a value near zero implies no significant interaction.

It's important to note that causal effects of interactions can differ on the ratio scale, which can have significant policy implications. This aspect underscores the importance of specifying not only the exposures and outcomes but also the effect measure and target population for generalization in causal studies.

[^11]: Note that causal effects of interactions often differ when measured on the ratio scale. This discrepancy can have significant policy implications, see: [@vanderweele2014]. Although beyond the scope of this article, it is worth emphasising again that when evaluating evidence for causality,  in addition to specifying the exposure and outcome, we must specify the measure of effect in which we are interested, as well as the target population for whom we wish to generalise [@hernán2004; @tripepi2007]. 
...
#| fig-cap: "Causal Diagram Illustrating Interaction: This diagram represents the individual and joint effects of two exposures, A and B, on outcome Y. It is assumed that A and B are causally independent. The diagram includes confounders L1 and L2, necessary for controlling backdoor paths related to both exposures. The counterfactual outcome is Y(a,b), and evidence for additive or subadditive interaction is indicated if E[Y(1,1) - Y(0,1) - Y(1,0) + Y(0,0)] ≠ 0. If B cannot be conceptualized as a variable amenable to intervention, this interaction is better described as effect modification. Note: Paths should be distinct and not intersect each other."
#| out-width: 60%
#| echo: false
...
Take, for example, the study of the causal effect of beliefs in Big Gods on social complexity. If we focus on whether this effect varies by region, geography becomes our 'effect modifier.' Imagine contrasting North American societies with Continental societies. Here, geography isn't treated as an intervention—it's conceptually implausible to do so. Instead, the aim is to explore if geography modifies the effect of a defined exposure (beliefs in Big Gods) on a specific outcome (social complexity).
...
[^12]: For distinctions within varieties of effect modification relevant for strategies of confounding control see [@vanderweele2007].
...
### Causal Mediation: The Limitations of Standard Approaches Exposed by Causal Diagrams

In the human sciences, mediation analysis is often mired in confusion, a situation exacerbated by the complex nature of causal relationships it aims to unravel. Beyond the intrinsic challenges of mediation analysis, much of the prevailing confusion stems from the prevalent use of structural equation models (SEMs), which are often derived from so-called 'structural equation graphs'. A critical issue here is that these models generally lack a systematic link to the counterfactual contrasts, which are the cornerstone of causal mediation analysis.

This disconnect leads to models that are statistically oriented but conceptually adrift from the true causal structures they intend to represent. Consequently, without a clear conceptual framework to assess structural relationships, these traditional approaches frequently yield ambiguous and scientifically questionable results. 

However, a solution lies in the thoughtful application of causal diagrams, combined with rigorous causal reasoning. By doing so, we can better identify and redress the shortcomings of current methodologies.
...
This total effect can be further broken down into direct and indirect effects, which constitute the mediated effect. To do this, we analyse the potential outcomes $Y(1)$ and $Y(0)$ in more detail:
...
Here, the effect of the exposure, set to $A = 1$, is considered along with the mediator assuming the value it naturally attains when the exposure is active.
...
This represents the effect when the exposure is inactive, $A = 0$, with the mediator taking its natural value in this scenario.

Now, let's further detail these components:
...
This breakdown of the total effect into natural direct and indirect effects provides clarity on the generic forms of the pre-specified estimands in causal mediation analysis. Without conceptualizing these targets as counterfactual contrasts, the application of statistical models to data can be misleading, as often occurs in structural equation modeling traditions. Contrary to their name, these models typically lack an inherent structural basis for interpretation and might be more aptly termed 'un-structural equation models.'

Approaching mediation from a structural perspective, as enabled by causal data sciences, allows for the decomposition of the Total Effect into parts mediated by changes in the mediator due to the treatment (NIE) and those that are unmediated (NDE). It is only with these targeted counterfactual contrasts in mind that we can effectively address questions of causal mediation to draw valid inferences, or understand the limitations when the stringent requirements for mediation analysis are not met, thereby justifying a more cautious approach. We next consider these assumptions.

### Chronological Causal Diagrams in Causal Mediation Analysis

Causal mediation analysis is governed by strict conditions, which chronologically ordered causal diagrams helps to clarify.   Consider again the hypothesis that cultural beliefs in 'Big Gods' influence social complexity, with political authority serving as a mediator. For argument's sake, let's assume these broad concepts are well-defined. What requirements are necessary to answer this hypothesis? 

1.  **No Unmeasured Exposure-Outcome Confounder**

This requirement is expressed: $Y(a,m) \coprod A | L1$. It means that after accounting for the covariates in set $L1$, there should be no unmeasured confounders influencing both cultural beliefs in Big Gods ($A$) and social complexity ($Y$). For instance, if our study examines the causal effect of cultural beliefs in Big Gods (the exposure) on social complexity (the outcome), and the covariates in $L1$ include factors like geographic location and historical context, we need to ensure that these covariates effectively block any confounding paths between $A$ and $Y$.  @fig-dag-mediation-assumptions shows this confounding path in brown.


2.  **No unmeasured mediator-outcome confounder**

This requirement is expressed: $Y(a,m) \coprod M | L2$. After controlling for the covariate set $L2$, we must ensure that no other unmeasured confounders affect the political authority $M$ and social complexity $Y$. For instance, if trade networks impact political authority and social complexity, to obstruct the unblocked path linking our mediator and outcome we must account for trade networks. Further, we must be entitled to assume the absence of any other confounders for the mediator-outcome path. @fig-dag-mediation-assumptions shows this confounding path is represented in blue.
...
Our discussion on causal mediation primarily focuses on how effects from two sequential exposures may combine to influence an outcome. This concept can be expanded to investigate the causal effects of multiple sequential exposures. In such cases, researchers often gravitate towards longitudinal growth models. However, a critical question arises: Where do counterfactuals fit within these models? Without incorporating counterfactuals, the depth and validity of the insights we derive become questionable. Chronologically arranged causal diagrams can be instrumental in highlighting the challenges and opportunities in these scenarios. They not only illuminate the intricacies involved in these longitudinal settings but also help in identifying potential pitfalls and opportunities. 
...
#| fig-cap: "Exposure confounder feedback is a problem for time-series models. Here, we do not assume that A_t1 affects L_t2. However, if an unmeasured variable (U_2) affects both the exposure A_t1 and the cofounder L_2, an unblocked path opens linking exposure to outcome (shown in red).  Again, we cannot infer causal effects in such scenarios by using regression-based methods because if we are 'damed-if-we-do-damned-if-we-do-not' condition on L_t2. In this setting, to address causal questions, we require G-methods."
#| out-width: 100%
#| echo: false
...
The issue of treatment confounder feedback poses significant challenges in evolutionary human science. Again, this problem is not adequately addressed by conventional regression-based methods, including multi-level models [@hernán2006; @robins1999; @robins1986]. The failure of regression stems from the necessity to condition on downstream confounders, which opens the door to a mix of collider and mediation biases in our estimates. As previously noted, G-methods encompass models suitable for analysing causal effects of both time-fixed and time-varying exposures [@naimi2017; @chatton2020; @hernán2006]. Recent advancements in targeted learning and other semi-parametric estimation methods also hold promise [@williams2021; @díaz2021; @breskin2021; @vanderlaan2018; @díaz2021; @wager2018; : @hernán2023 @díaz2021 @vanderweele2015 @hoffman2022 @hoffman2023 @chatton2020 @shiba2021; @sjölander2016; @breskin2020; @vanderweele2009a; @vansteelandt2012; @shi2021]; however, these methods have not yet gained widespread acceptance in human evolutionary sciences. I anticipate the scene will soon change. Causal diagrams help to clarify the structure of this problems, and the demand both for accurately measured time-series data and appropriate methods for counterfactual recovery.[^help] [^15]

[^help]: Useful overviews include: @hernán2023 @díaz2021 @vanderweele2015 @hoffman2022 @hoffman2023 @chatton2020 @shiba2021 @sjölander2016 @breskin2020 @vanderweele2009a @vansteelandt2012 @shi2021.
...
**Part 1:** introduced the counterfactual framework in causal data science, highlighting the necessity of contrasting the observed world with potential alternatives for accurate estimation of causal effects. This approach requires clear, explicit assumptions and a detailed workflows. It is only within a framework of assumptions and workflows that causal diagrams enable the assessment of structural assumptions essential for addressing the 'no-unmeasured' confounding assumption. 

**Part 2:** described essential concepts and introduced core strategies for applying causal diagrams to control confounding. I emphasised the value of maintaining chronological order in these diagrams to accurately mirror the assumed causal sequences. This approach not only deepens our understanding of confounding but also highlights specific data requirements. For instance, by measuring confounders prior to exposures, we can avert common issues like mediator bias and post-stratification bias.

**Part 3:** merged the counterfactual framework with causal diagrams to the counterfactual framework with causal diagrams to demystify the the concepts of causal interaction, causal mediation, and dynamic longitudinal feedback. We discovered that causal interaction can either mean a combined effect of joint interventions or an effect-modification across subpopulations. To properly interpret 'interaction', then, it is crucial to specify our causal estimands in advance. We found that causal mediation involves a dual exposure, and that identification problem here is subject to stringent assumptions. We considered that for complex time series data, confounder-treatment feedback can lead to identification problems that elude standard methods. Throughout these discussions causal diagrams plainly elucidated the prospects and challenges for addressing more complex causal questions such as those that involve more that one intervention.

The adoption of causal inferential methodologies in many human sciences lags. Traditional correlational methods continue to hold sway. A broader shift towards causal methodologies is essential for advancing scientific research. That much of the foundational work has been done offers reasons for hope.

However, the need for accurate time-series data and the time it will take for many researchers to 're-tool' carries implications for research design, funding, and the tolerated pace of scientific discovery. The human sciences will need to transition from an output-focused research model to one that prioritises comprehensive retraining and substantial time-series data collection. This shift is essential for the effective application of causal data science for enhancing our understanding of complex psychological and cultural phenomena.
