...
mice_donate_mids <- mice_donate_c %>%
  ...
  mutate(t0_eth_cat = as.factor(t0_eth_cat),
    ...
    #    t0_volunteers = as.numeric(ifelse(t0_hours_charity > 1,1,0)),
        t2_volunteers = as.integer(ifelse(t2_hours_charity > 1,1,0)),
        t2_hours_charity_ord = as.integer(round(t2_hours_charity)), 
        t2_charity_donate_ord = as.integer(round(t2_charity_donate))) |> 
...
str(mice_donate_long$t2_volunteers)


table(mice_donate_long$t2_hours_charity_ord)
str(mice_donate_long$t2_hours_charity_ord)
...
mice_donate_long$t2_volunteers
...
# 
# 
# est_RD <- transform(sim.att, RD = `E[Y(four_up)]` - `E[Y(zero)]`)
# 
# 
# 
# 
# sim_estimand <- sim_ame(sim.imp, var = X, cl = cores, verbose = FALSE)
# est_rr <- transform(sim_estimand, RR = `E[Y(four_up)]` / `E[Y(zero)]`, 
...
# # poisson outcome
#                 Estimate   5 %  95 %
# E[Y(zero)]         1.577 1.521 1.884
# E[Y(one)]          1.343 1.404 2.491
# E[Y(less_four)]    1.823 1.851 3.502
# E[Y(four_up)]      2.120 2.074 2.731
# RD                 0.543 0.292 1.078
# RR                 1.344 1.159 1.677
...
# test_dev <- causal_contrast_engine(df = dt_donate,  # note change
#   Y = "t2_volunteers",
#   X = X, 
...
mod_donate_volunteer_rr  <- double_robust(
...
mod_donate_hours_charity_rd  <- double_robust(
  df = dt_donate, 
  Y = "t2_hours_charity_log_z",
  X = X,
  baseline_vars = baseline_vars_donate,
...
mod_donate_hours_charity_rd
saveRDS(mod_donate_hours_charity_rd, here::here(push_mods, "mod_donate_hours_charity_rd"))
...
mod_donate_charity_donate_rd  <- double_robust(
  df = dt_donate,  # note change
  Y = "t2_charity_donate_log_z",
  X = X,
  baseline_vars = baseline_vars_donate,
  ...
  splines = FALSE, 
  vcov = "HC",
  new_name = "Charity Donations (log z)"
)


mod_donate_charity_donate_rd

saveRDS(mod_donate_charity_donate_rd, here::here(push_mods, "mod_donate_charity_donate_rd"))
