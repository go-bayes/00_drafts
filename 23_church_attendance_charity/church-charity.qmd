...
dt_19 <- dat_long |> 
  filter( wave == "2019") |> 
  select(id, wave, religion_church2) 
hist( dt_19$religion_church2, breaks = 100 )
# 
# quantile_breaks <- quantile(dt_19$religion_church2, probs = seq(0, 1, 1/4), na.rm = TRUE)
# quantile_breaks
# 
...
# 
# quantile_breaks
# dt_19$religion_church2_quartile <- cut(dt_19$religion_church2,
...
table(dt_19$religion_church_coarsen)
...
baseline_vars_practical = mice_practical_long |> 
  dplyr::select(starts_with("t0"), -c("t0_extraversion_quantile"))|> colnames() # strange to include these -- as they  are income by other names
