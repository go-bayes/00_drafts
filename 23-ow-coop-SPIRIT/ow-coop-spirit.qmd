...
push_mods <-
  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23-ow-coop-spirit"
  )
...
dat_long <- dat |>
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_believe_spirit), 1, 0))|>   # creating an indicator for the first wave. Inclusion criteria
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
  dplyr::mutate(k_19 =  ifelse(
    wave == 2019 &
      year_measured == 1 &
      !is.na(religion_believe_spirit),#  Inclusion criteria
    1, 0)) |>   # creating an indicator for the first wave; note that we allow people t
...
#  filter(id %in% religious_ids) %>%   only if subsetting religious ids 
  ...
  # ) %>%
  mutate(
    religion_believe_spirit_n = as.integer(religion_believe_spirit),
  ) |>
  mutate(
    religion_believe_spirit = as.factor(religion_believe_spirit), # make factor for impuatation
...
exposure_var = c("religion_believe_spirit") # we could construct this after imputation. # "perfectionism_high" to be replace by "perfectionism_coarsen" - do the data wrangling after imputation.
...
## data for mice
exposure_vars = c("t1_religion_believe_spirit")
#exposure_vars
...
mice_support_mids <- mice_support_c %>%
  ...
  dplyr::select(-t0_family_time, -t0_friends_time, -t0_community_time#, #-t0_family_money, -t0_friends_money,-t0_community_money
...
# Set exposure 
X <- here_read("exposure_vars") # get exposure variable
...
love.plot(mice_warmth_ebal, binary = "std", thresholds = c(m = .1), var.order = "unadjusted")
...
# settings 
# dataframe
dt_support = match_support_ebal #mice_support_ebal

# cores
cl =  parallel::detectCores () 
cores =  parallel::detectCores () 

levels(dat_long_t$religion_believe_spirit)
nsims = 1000
...
out <- msm::statetable.msm(religion_believe_spirit, id, data = dt_positivity_full)
...
title = "ATE: +1 year causal effect from gain of belief in Spirit/Life Force"
...
ggsave(
  ...
  height = 4.5,
  units = "in",
  filename = "group_plot_virtue-rd-SPIRIT.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
subtitle_support_rr = "Volunteers"
title = title
...
ggsave(
  ...
  height = 4.5,
  units = "in",
  filename = "gr_plot_virtue_rr-SPIIRIT.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600)
```
...
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_warmth_rd-SPIRIT.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
ggsave(
  ...
  height = 4.5,
  units = "in",
  filename = "group_plot_support_perceived-SPIRIT.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
ggsave(
  group_plot_ate_support_rr,
  path = here::here(here::here(push_mods, "group_plot_ate_support_received_SPIRIT")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "group_plot_ate_support_received_SPIRIT.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
