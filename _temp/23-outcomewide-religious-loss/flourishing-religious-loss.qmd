---
title: "Effect of religious de-identification on multi-dimensionsal well-being"
subtitle: ""
abstract: |
...
dat_long <- dat |>
 ...
 dplyr::filter(id != 9630) %>% # problematic reports no income but works full time and owns home.
  ...
  mutate(religion_religious = as.integer(as.numeric(as.character(religion_religious))) - 1) |> 
  ...
  dplyr::mutate(meets_criteria_baseline = if_else(religion_religious_not == 1, 1, 0)) |> 
  arrange(id) |>
  filter((wave == 2018 & year_measured == 1) |
           (wave == 2019 & year_measured == 1) |
           (wave == 2020)) %>%
  group_by(id) |> 
  mutate(k_18 = ifelse(wave == 2018 & ! is.na(religion_religious_not) &  meets_criteria_baseline == 1, 1, 0)) %>% # selection criteria
