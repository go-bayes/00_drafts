...
# for saving models
push_mods <-
  fs::path_expand("/Users/joseph/v-project\ Dropbox/Joseph\ Bulbulia/outcomewide/who_nfd/mods")

# for saving figures
push_figs <-
  fs::path_expand(" /Users/joseph/v-project\ Dropbox/Joseph\ Bulbulia/outcomewide/who_nfd/figs")
...
gt <- df1 %>%
  ...
  dplyr::rename(NZdep  =  NZDep2013 ) |> # to keep old code consistent
  ...
  dplyr::mutate(ChildrenNum.log.C = scale(log(ChildrenNum + 1), scale =
                                            F, center = T)) %>%
  dplyr::mutate(Male = factor(Male))C)
...
# #CHRISTIANS MOVING MORE TO NDF (SUPPORTS DISENCHANTMENT)
...
table(gu1$Christian_nfd_3)


# prepare data for HMM.  # NOT STABLE
# gu1 <- gu %>%
...
#
# #sc_initmod
# saveRDS(sc_initmod, here::here(push_mods, "nfd_sc_initmod") )
#
#
# # fit model
...
# # save model
# nfd_1 <-sc_fit
# saveRDS(nfd_1, here::here(push_mods, "nfd_1") )
#
#
...
#      title ="Hidden Markov Model Christian NFD/ Christian / Secular Affiliation:2009-2021",
#     # loops=T,
#      vertex.size = 40,
#      legend.prop =.3,
#      ncol.legend = 2)
...
saveRDS(gh.mod.mnm, here::here(push_mods,"gt_ndf_msm"))
m1<-gh.mod.mnm
m1 <-readRDS( here::here(push_mods,"gt_ndf_msm"))
m1
...
size <- c(width, height) / res
dev.off()
png("myplot.png",  width = width*3, height = height*3, units = "px", res = 300)
plotmat(toz,
        cex.txt = 0.9,
        box.size = 0.09,
        box.type = "circle",
...
# Reorder variable names by time index
library(ggdag)
...
# Calculate angle and control points for curved edges


arc_fun <- function(data) {
  x1 <- data$x[1]
  y1 <- data$y[1]
  x2 <- data$xend[1]
  y2 <- data$yend[1]
  if (abs(x1 - x2) > 1 | y1 != y2) {
    data.frame(x = x1, y = y1, xend = x2, yend = y2, curvature = 0)
  } else {
    dx <- x2 - x1
    dy <- y2 - y1
    mx <- (x1 + x2) / 2
    my <- y1 + ifelse(dy > 0, 1, -1) * sqrt(dx * dx / 4 + dy * dy / 4)
    cx1 <- (x1 + mx) / 2
    cy1 <- y1
    cx2 <- (x2 + mx) / 2
    cy2 <- y2
    curve <- sprintf("M %f,%f C %f,%f %f,%f %f,%f",
                     x1, y1, cx1, cy1, cx2, cy2, x2, y2)
    data.frame(x = x1, y = y1, xend = x2, yend = y2, curvature = 0.2, path = curve)
  }
}
