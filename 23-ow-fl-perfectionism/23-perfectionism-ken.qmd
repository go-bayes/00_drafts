...
env_names <- dat |> 
  select(starts_with("env_")) |> 
  colnames()

dat$equake_affected_earthquake

# inspect exposure
hist(dat$perfectionism, breaks = 100)


# quartile not used: quartiles may change annually
...
# use coarsen instead of quartile
...
hist( dat_long$perfectionism, breaks = 50) 
...
dat_long[ , perfectionism_coarsen := cut(perfectionism,
                        breaks = c(1,4,5,7), 
                        include.lowest = TRUE,
...
dat_long[ , perfectionism_coarsen_crude := cut(perfectionism,
                        breaks = c(1,4,7), 
                        include.lowest = TRUE,
                        include.highest = TRUE,
                        na.rm = TRUE,
...
dt_18_19 <- dat_long |> 
  ...
  select(id, wave, perfectionism, perfectionism_coarsen, perfectionism_coarsen_crude) |> 
...
table(dat_long$perfectionism_coarsen)
...
# we might think of three states, and compare movements only from the highest to the lowest. 
...
# arguably too coarse.
msm::statetable.msm(round(perfectionism_coarsen_crude_n, 0), id, data = dt_18_19) |>
  kbl() |>
  kable_paper(full_width = F)
...
exposure_var = c("perfectionism") # we could construct this after imputation. # "perfectionism_high" to be replace by "perfectionism_coarsen" - do the data wrangling after imputation.
...
# # Calculate the quantile breaks (cut points)
# quantile_breaks <- quantile(mc$t0_perfectionism, probs = seq(0, 1, 1/4), na.rm = TRUE)
# 
# # Create labels based on the cut points in the desired format
# cut_labels <- paste0(quantile_breaks[-length(quantile_breaks)], "_", quantile_breaks[-1])
# 
# cut_labels
# # Create the ordered factor variable with the new labels
# mc$t0_perfectionism_quartile <- cut(mc$t0_perfectionism,
#                                         breaks = quantile_breaks,
#                                         labels = cut_labels,
#                                         ordered_result = TRUE,
#                                         include.lowest = TRUE)
# 
# mc$t1_perfectionism_quartile <- cut(mc$t1_perfectionism,
#                                         breaks = quantile_breaks,
#                                         labels = cut_labels,
#                                         ordered_result = TRUE,
#                                         include.lowest = TRUE)
...
dat_18 <- readRDS(here::here(push_mods, "dat_18"))

# if we need weights
newdat <- data.frame( rep(dat_18$sample_weights, N))

# do not use the name "weigths" as will not play well with matchit/weightit
...
# needed only for three wave within sube
...
ml_health<- mc_v %>%
  ...
  mutate(t0_eth_cat = as.factor(t0_eth_cat),
         ...
         # t1_perfectionism_high =. Not used - to delete as.factor(t1_perfectionism_high), 
          t1_perfectionism_coarsen = cut(t1_perfectionism,
                        breaks = c(1,4,5,7), 
                        include.lowest = TRUE,
                        include.highest = TRUE,
                        na.rm = TRUE,
...
table(mf_health$t1_perfectionism_coarsen)
...
ml_embodied<- mc_v %>%
  ...
  mutate( 
         ...
         t0_rural_gch2018 = as.factor(t0_rural_gch2018), # t1_perfectionism_high =. Not used - to delete as.factor(t1_perfectionism_high), 
          t1_perfectionism_coarsen = cut(t1_perfectionism,
                        breaks = c(1,4,5,7), 
                        include.lowest = TRUE,
                        include.highest = TRUE,
...
# 
# # need to rename sample_weights - for contin mods no prop scores
# ml_embodied_cont <- mc_v %>%
#   dplyr::mutate(id = as.factor(rep(1:n_id, 11))) |> # needed 
#   mutate( 
#          t0_eth_cat = as.factor(t0_eth_cat),
#          t0_rural_gch2018 = as.factor(t0_rural_gch2018),
...
#     .id
#   ) |> 
#   rename(weights = sample_weights) |> 
#   relocate(weights, .before = starts_with("t1_"))  %>%
#   relocate(id, .before = weights)  %>%
#   relocate(starts_with("t0_"), .before = starts_with("t1_"))  %>%
#   relocate(starts_with("t2_"), .after = starts_with("t1_"))  %>%
#   relocate(starts_with("t3_"), .after = starts_with("t3_"))  %>%
#   mutate_if(is.matrix, as.vector) %>%
...
#mf_embodied_cont <- mice::complete(ml_embodied_cont, "long", inc = TRUE)


saveRDS(ml_embodied, here::here(push_mods, "ml_embodied"))
saveRDS(mf_embodied, here::here(push_mods, "mf_embodied"))

# saveRDS(ml_embodied_cont, here::here(push_mods, "ml_embodied_cont"))
# saveRDS(mf_embodied_cont, here::here(push_mods, "mf_embodied_cont"))
```
...
ml_social<- mc_v %>%
  ...
  mutate(t0_eth_cat = as.factor(t0_eth_cat),
         t0_rural_gch2018 = as.factor(t0_rural_gch2018),
         t0_charity_donate = log(t0_charity_donate + 1), # t1_perfectionism_high =. Not used - to delete as.factor(t1_perfectionism_high), 
          t1_perfectionism_coarsen = cut(t1_perfectionism,
                        breaks = c(1,4,5,7), 
                        include.lowest = TRUE,
                        include.highest = TRUE,
...
baseline_vars_embodied = mf_embodied |> 
  dplyr::select(starts_with("t0"), 
                -t0_perfectionism_z, 
                -t0_kessler_6_z) |> colnames() # note we added quartiles for this analysis to demonstrate interest of different causal estimans
...
#'
#' Estimates causal contrasts using generalized linear models (GLM) for different types of treatment effects (ATE, ATT)
#' and outcomes (RR, RD). Supports handling multiply imputed datasets (`mids`) and allows flexibility in model specification.
#'
...
causal_contrast_engine <- function(df, Y, X, baseline_vars, treat_0, treat_1, estimand = c("ATE", "ATT"), type = c("RR", "RD"), nsims = 200, cores = parallel::detectCores(), family = "gaussian", weights = TRUE, continuous_X = FALSE, splines = FALSE, vcov = "HC2", verbose = FALSE) {
    # Check required packages
    required_packages <- c("clarify", "rlang", "glue", "parallel")
    for (pkg in required_packages) {
        if (!requireNamespace(pkg, quietly = TRUE)) {
            stop("Package '", pkg, "' is needed but not installed.")
        }
    }
  
    # Validate the family argument
    if (is.character(family) && !family %in% c("gaussian", "binomial", "Gamma", "inverse.gaussian", "poisson", "quasibinomial", "quasipoisson", "quasi")) {
        stop("Invalid 'family' argument. Please specify a valid family function.")
    }
    family_fun <- if (is.character(family)) get(family, mode = "function", envir = parent.frame()) else family

    # Use the construct_formula function to build the formula string
    ...
    model_fits <- if ("wimids" %in% class(df)) {
        purrr::map(complete(df, "all"), function(d) {
            glm(as.formula(formula_str), family = family_fun, data = d, weights = if (weights) d$weights else NULL)
        })
    } else {
        list(glm(as.formula(formula_str), family = family_fun, data = df, weights = if (weights) df$weights else NULL))
    }

    # Process simulation for marginal effects
    sim.imp <- misim(model_fits, n = nsims, vcov = vcov)
    sim_estimand <- sim_ame(sim.imp, var = X, cl = cores, verbose = verbose)

    # Generate and return the summary of simulated estimands
...
#| label: models social
#| eval: false
ml_social <- readRDS(here::here(push_mods, "ml_social"))

# longform data if necessary
mf_social <- readRDS(here::here(push_mods, "mf_social"))

colnames(mf_social)

mf_social$t1_perfectionism_high
...
baseline_vars_social = mf_social |>
  dplyr::select(starts_with("t0"),
                -t0_belong_z,
                -t0_support_z) |> colnames()

options(scipen = 999)
baseline_vars_social

dt_match_social <- match_mi(data = ml_social, X = "t1_perfectionism_high", baseline_vars = baseline_vars_social , estimand = "ATE",  method = "ebal", sample_weights = "sample_weights")

saveRDS(dt_match_social, here::here(push_mods, "dt_match_social"))

sum <- summary(dt_match_social)
sum
plot(sum)
bal.tab(dt_match_social)

dt_match_trim_social <- WeightIt::trim(dt_match_social, at = .999)
sum_trim <- summary(dt_match_trim_social)
sum_trim

bal.tab(dt_match_trim_social) # 12

love.plot(dt_match_trim_social, binary = "std", thresholds = c(m = .1))
love.plot(dt_match_social, binary = "std", thresholds = c(m = .1))

dev.off()


# settings 
dt_match_social_use = dt_match_trim_social # matched dataset from propensity scores.  If no propensity score model make ml_match = ml
...
mod_social_belong  <- gcomp_sim(
  df = dt_match_social_use,  # note change
  Y = "t2_belong_z",
  X = "t1_perfectionism_high",
...
mod_social_belong_accept  <- gcomp_sim(
  ...
  continuous_X = FALSE,
  splines = FALSE, 
  new_name = "support_belong_accept"
)
...
mod_social_belong_routsider  <- gcomp_sim(
  ...
  continuous_X = FALSE,
  splines = FALSE, 
  new_name = "support_belong_outsider (reversed)"
)
...
mod_social_belong_beliefs  <- gcomp_sim(
  ...
  continuous_X = FALSE,
  splines = FALSE, 
  new_name = "support_belong_beliefs"
)
...
```{r}
#| label: model embodied continuous
#| eval: false
...
# scale
mean(dt_19$perfectionism, na.rm = TRUE) # 3.085961
sd(dt_19$perfectionism, na.rm = TRUE) # 1.375789

ml_embodied_cont <- readRDS(here::here(push_mods, "ml_embodied_cont"))

# longform data if necessary
mf_embodied_cont <- readRDS(here::here(push_mods, "mf_embodied_cont"))

colnames(mf_embodied_cont)

mf_embodied_cont$t1_perfectionism_z
...
baseline_vars_embodied_cont = mf_embodied_cont |> 
  dplyr::select(starts_with("t0"), 
                # -t0_perfectionism_z, 
                -t0_perfectionism_high_z,
                -t0_kessler_6_z,
                -t0_perfectionism_quartile) |> colnames() # note we added quartiles for this analysis to demonstrate interest of different causal estimans


options(scipen = 999)
baseline_vars_embodied_cont
...
baseline_vars = baseline_vars_embodied_cont
# outcomes


mod_embodied_hlth_fatigue_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_hlth_fatigue_z",
  X =  X,
  baseline_vars = baseline_vars,
  ...
  family = gaussian,
  weights = TRUE,
  continuous_X = TRUE, # new setting
  splines = TRUE, # new setting
  new_name = "hlth_fatigue_z"
)
...
# save model
saveRDS(mod_embodied_hlth_fatigue_cont, here::here(push_mods, "mod_embodied_hlth_fatigue_cont"))

# rumination
mod_embodied_rumination_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_rumination_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_rumination_cont, here::here(push_mods, "mod_embodied_rumination_cont"))

## kessler_depressed
mod_embodied_kessler_depressed_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_depressed_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_depressed_cont, here::here(push_mods, "mod_embodied_kessler_depressed_cont"))



## kessler_effort
mod_embodied_kessler_effort_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_effort_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_effort_cont, here::here(push_mods, "mod_embodied_kessler_effort_cont"))


## kessler_hopeless
mod_embodied_kessler_hopeless_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_hopeless_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_hopeless_cont, here::here(push_mods, "mod_embodied_kessler_hopeless_cont"))


## kessler_nervous

mod_embodied_kessler_nervous_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_nervous_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_nervous_cont, here::here(push_mods, "mod_embodied_kessler_nervous_cont"))


## kessler_restless
mod_embodied_kessler_restless_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_restless_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_restless_cont, here::here(push_mods, "mod_embodied_kessler_restless_cont"))


## kessler_worthless
mod_embodied_kessler_worthless_cont  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_worthless_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_worthless_cont, here::here(push_mods, "mod_embodied_kessler_worthless_cont"))


## t2_kessler_6
mod_embodied_kessler_6_cont  <- gcomp_sim(
   df = df,  # note change
  Y = "t2_kessler_6_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_6_cont, here::here(push_mods, "mod_embodied_kessler_6_cont"))
...
group_tab_embodied_cont <- group_tab(tab_embodied_cont, type = "RD")

group_tab_embodied_cont



group_plot_ate_embodied_cont <- group_plot_ate(
  group_tab_embodied_cont,
  type = "RD",
  title = "ATE of High Perfectionism Discrepency",
  subtitle = "Embodied Outcomes - Continuous X",
  xlab = "(sd units)",
  ylab = "test",
  x_offset = -.5,
  x_lim_lo = -.5,
...
group_plot_ate_embodied_cont



ggsave(
  group_plot_ate_embodied_cont,
  path = here::here(here::here(push_mods, "group_plot_ate_embodied_cont")),
  width = 8,
  height = 4,
  units = "in",
  filename = "group_plot_ate_embodied_cont.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
```{r}
#| label: model embodied quartiles
#| eval: false
...
table( mf_embodied$t1_perfectionism_quartile )/11

# Set exposure 
X <- "t1_perfectionism_quartile"


# baselin vars ---------------------------------------------------------

# remove variables from baseline that are not relevant
baseline_vars_embodied_quart = mf_embodied |> 
  dplyr::select(starts_with("t0"), 
               # -t0_perfectionism_z,  # use cont at baseline
                -t0_perfectionism_high_z,
                -t0_kessler_6_z,
                -t0_perfectionism_quartile
                ) |> colnames() # note we added quartiles for this analysis to demonstrate interest of different causal estimans

baseline_vars_embodied_quart
# options(scipen = 999)
# 
# mf_embodied$t1_perfectionism_quartile
# att



dt_match_embodied_quart <-
  match_mi(
    data = ml_embodied,
    X = "t1_perfectionism_quartile",
    baseline_vars = baseline_vars_embodied_quart,
    estimand = "ATE",
    method = "ebal",
...
saveRDS(dt_match_embodied_quart , here::here(push_mods, "dt_match_embodied_quart"))

sum <- summary(dt_match_embodied_quart)
sum
plot(sum)
bal.tab(dt_match_embodied_quart)

dt_match_trim_embodied_quart <- WeightIt::trim(dt_match_embodied_quart, at = .995)
sum_trim <- summary(dt_match_trim_embodied_quart)
sum_trim

bal.tab(dt_match_trim_embodied_quart)

love.plot(dt_match_trim_embodied_quart, binary = "std", thresholds = c(m = .1))
love.plot(dt_match_embodied_quart, binary = "std", thresholds = c(m = .1))

dev.off()

ml_embodied$t1_perfectionism

df = dt_match_trim_embodied_quart
...
mod_embodied_hlth_fatigue_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_hlth_fatigue_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_hlth_fatigue_quart, here::here(push_mods, "mod_embodied_hlth_fatigue_quart"))

# rumination
mod_embodied_rumination_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_rumination_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_rumination_quart, here::here(push_mods, "mod_embodied_rumination_quart"))

## kessler_depressed
mod_embodied_kessler_depressed_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_depressed_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_depressed_quart, here::here(push_mods, "mod_embodied_kessler_depressed_quart"))



## kessler_effort
mod_embodied_kessler_effort_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_effort_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_effort_quart, here::here(push_mods, "mod_embodied_kessler_effort_quart"))


## kessler_hopeless
mod_embodied_kessler_hopeless_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_hopeless_z",
  X = X,
  baseline_vars = baseline_vars_embodied,
...
# save model
saveRDS(mod_embodied_kessler_hopeless_quart, here::here(push_mods, "mod_embodied_kessler_hopeless_quart"))


## kessler_nervous
mod_embodied_kessler_nervous_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_nervous_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_nervous_quart, here::here(push_mods, "mod_embodied_kessler_nervous_quart"))


## kessler_restless
mod_embodied_kessler_restless_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_restless_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_restless_quart, here::here(push_mods, "mod_embodied_kessler_restless_quart"))


## kessler_worthless
mod_embodied_kessler_worthless_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_worthless_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_worthless_quart, here::here(push_mods, "mod_embodied_kessler_worthless_quart"))

## t2_kessler_6
mod_embodied_kessler_6_quart  <- gcomp_sim(
  df = df,  # note change
  Y = "t2_kessler_6_z",
  X = X,
  baseline_vars = baseline_vars,
...
# save model
saveRDS(mod_embodied_kessler_6_quart, here::here(push_mods, "mod_embodied_kessler_6_quart"))
...
group_tab_embodied_quart <- group_tab(tab_embodied_quart, type = "RD")

group_tab_embodied_quart



group_plot_ate_embodied_quart <- group_plot_ate(
  group_tab_embodied_quart,
  type = "RD",
  title = "ATE of High Perfectionism Discrepency",
  subtitle = "Embodied Outcomes: from lowest (score 1-2) to highest (score 4-7) quartile",
  xlab = "(sd units)",
  ylab = "test",
  x_offset = -.5,
  x_lim_lo = -.5,
...
group_plot_ate_embodied_quart

dev.off()
ggsave(
  group_plot_ate_embodied_quart,
  path = here::here(here::here(push_mods, "group_plot_ate_embodied_quart")),
  width = 8,
  height = 4,
  units = "in",
  filename = "group_plot_ate_embodied_quart",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
group_plot_ate_embodied_quart <- group_plot_ate(
  group_tab_embodied_quart,
  type = "RD",
  title = "ATE of High Perfectionism Discrepency: Quartile",
  subtitle = "Embodied Outcomes: from lowest (score 1-2) to highest (score 4-7) quartile",
  xlab = "(sd units)",
  ylab = "test",
  x_offset = -.5,
...
group_plot_ate_embodied_bin <- group_plot_ate(
  group_tab_embodied,
  type = "RD",
  title = "ATE of High Perfectionism Discrepency: Binary Cut > 4",
  subtitle = "Embodied Outcomes: Low (1-4) to High (>4)",
  xlab = "(sd units)",
  ylab = "test",
  x_offset = -.5,
  x_lim_lo = -.5,
...
group_plot_ate_embodied_cont <- group_plot_ate(
  group_tab_embodied_cont,
  type = "RD",
  title = "ATE of High Perfectionism Discrepency: Continuous Mean to + 1",
  subtitle = "Embodied Outcomes: Mean (3.086) + SD (1.376)",
  xlab = "(sd units)",
  ylab = "test",
  x_offset = -.5,
  x_lim_lo = -.5,
...
library(patchwork)
graph_compare_approaches_embodied <- group_plot_ate_embodied_bin / group_plot_ate_embodied_cont /group_plot_ate_embodied_quart + 
  plot_annotation(title = "Comparison of Causal Effect Estimatation Approaches", tag_levels = "a")

graph_compare_approaches_embodied

ggsave(
  graph_compare_approaches_embodied,
  path = here::here(here::here(push_mods, "graph_compare_approaches_embodied")),
  width = 8,
  height = 12,
  units = "in",
  filename = "graph_compare_approaches_embodied.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
