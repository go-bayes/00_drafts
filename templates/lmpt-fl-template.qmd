---
title: "Causal effects of meaning of XXXX on multi-dimensional well-being"
...
# libraries for jb (when internet is not accessible): comment out and use above
# read libraries
source("/Users/joseph/GIT/templates/functions/libs2.R")

# read functions: # for jb: comment out and use above
# read libraries
...
push_mods <-
  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23-template"
  )
...
## set nzavs exposure: example here
...
# which confounder set will reduce bias? 
...
dat_long <- dat |>
 ...
 dplyr::filter(id != 9630) %>% # problematic reports no income but works full time and owns home.
  ...
  mutate(religion_believe_god = as.integer(as.numeric(as.character(
  ...
  dplyr::select(
    ...
    "age",
    "born_nz",
    "hlth_disability", # important confounder
  ...
  filter((wave == 2018 & year_measured == 1) |
           ...
           (wave == 2020)) %>%
  group_by(id) |> 
  mutate(k_18 = ifelse(wave == 2018 &  !is.na(!!sym(nzavs_exposure)), 1, 0)) %>% # selection criteria
  mutate(h_18 = mean(k_18, na.rm = TRUE)) %>%
  mutate(k_19 = ifelse(wave == 2019 & !is.na(!!sym(nzavs_exposure)), 1,0)) %>% # selection criteria
...
# check unique n
n_unique(dat_long$id) # 34761 # sample size

table1(~ religion_church_binary|wave, data = dat_long)
...
result_table <- dat_long %>%
  count(wave, get(nzavs_exposure)) %>%
  pivot_wider(names_from = get(nzavs_exposure), values_from = n, values_fill = 0) %>%
  as.table()
