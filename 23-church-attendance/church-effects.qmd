---
title: "Outcome-Wide Income Effects"
subtitle: ""
abstract: |
...
push_mods <-
  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23_income"
  )
...
dat_long <- dat |> 
  ...
  select("wave", 
    ...
    "religion_church_binary", # at least once per month
...
# I am hardly ever satisfied with my performance.
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                !is.na(household_inc) & household_inc > 30975, 1, 0)) |>   # creating an indicator for the first wave
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
  dplyr::mutate(k_19 =  ifelse(wave == 2019 &
                                 year_measured == 1 &
                                  !is.na(household_inc) == 1 & household_inc > 30975, 1, 0)) |>   # creating an indicator for the first wave; note that we allow people t
...
length(unique(dat_long$id))# 28518
...
quantile_breaks <- quantile(dt_19$household_inc, probs = seq(0, 1, 1/4), na.rm = TRUE)
...
quantile_breaks
dt_19$household_inc_quartile <- cut(dt_19$household_inc,
...
dt_18_19 <- dat_long |> 
  filter(wave == "2018" | wave == "2019") |> 
  mutate(
    household_inc_quantile =  cut(
      household_inc,
      breaks = c(30975, 74000, 110000, 160000, 4000001),
      include.lowest = TRUE,
      include.highest = TRUE,
      na.rm = TRUE,
      right = FALSE
    ))|>
  mutate(household_inc_quantile = factor(
      household_inc_quantile,
      levels = c("[3.1e+04,7.4e+04)", "[7.4e+04,1.1e+05)", "[1.1e+05,1.6e+05)", "[1.6e+05,4e+06]"),
      labels = c("q1", "q2", "q3", "q4"),
      ordered = TRUE
    )  ) |> 
  mutate(household_inc_quantile_n = as.numeric( household_inc_quantile)) |> 
  select(id, wave, household_inc_quantile, household_inc_quantile_n) #|> 
  # mutate(perfectionism_coarsen_n = as.numeric(perfectionism_coarsen),
  #        perfectionism_coarsen_crude_n = as.numeric( perfectionism_coarsen_crude))

table(dt_18_19$household_inc_quantile)


out <- msm::statetable.msm(round(household_inc_quantile_n, 0), id, data = dt_18_19)

# for a function I wrote to create state tables
state_names <- c("30k-74k", "74k-110k", "110k-160k", "over-160k")
...
dt_19_n <- dat_long |> 
  ...
  # mutate(perfectionism_coarsen_n = as.numeric(perfectionism_coarsen),
  #        perfectionism_coarsen_crude_n = as.numeric( perfectionism_coarsen_crude))
third_breaks <- quantile(dt_19_n$household_inc, probs = seq(0, 1, 1/3), na.rm = TRUE)

# Create labels based on the cut points in the desired format
cut_labels <- paste0(third_breaks[-length(third_breaks)], "_", third_breaks[-1])

third_breaks
dt_19_n$household_inc_thirds <- cut(dt_19_n$household_inc,
                                        breaks = third_breaks,
                                        labels = cut_labels,
                                        ordered_result = TRUE,
                                        include.lowest = TRUE)
...
dt_18_19_third <- dat_long |> 
  filter(wave == "2018" | wave == "2019") |> 
  mutate(
    household_inc_thirds =  cut(
      household_inc,
      breaks = c(31000, 85000, 145000, 4000001),
      include.lowest = TRUE,
      include.highest = TRUE,
      na.rm = TRUE,
      right = FALSE
    ))|>
 # mutate(household_inc_thirds = factor(
   #   household_inc_thirds,
     # levels = c("[3.1e+04,7.4e+04)", "[7.4e+04,1.1e+05)", "[1.1e+05,1.6e+05)", "[1.6e+05,4e+06]"),
  #    labels = c("low", "med", "high"),
  #    ordered = TRUE
  #  )  ) |> 
  mutate(household_inc_thirds_n = as.numeric( household_inc_thirds)) |> 
  select(id, wave, household_inc_thirds, household_inc_thirds_n) #|> 
  # mutate(perfectionism_coarsen_n = as.numeric(perfectionism_coarsen),
  #        perfectionism_coarsen_crude_n = as.numeric( perfectionism_coarsen_crude))

table(dt_18_19$household_inc_thirds)


out <- msm::statetable.msm(round(household_inc_thirds_n, 0), id, data = dt_18_19_third)
...
exposure_var = c("household_inc_quantile", "household_inc") # we could construct this after imputation. # "perfectionism_high" to be replace by "perfectionism_coarsen" - do the data wrangling after imputation.
...
exposure_vars = "t1_household_inc_quantile"
...
prep_health_multiple <- create_filtered_wide_dataframes(prep_health, exposure_vars = "t1_household_inc_quantile") 
...
prep_embodied_multiple <- create_filtered_wide_dataframes(prep_embodied, exposure_vars = "t1_household_inc_quantile") 



prep_practical_multiple <- create_filtered_wide_dataframes(prep_practical, exposure_vars = "t1_household_inc_quantile") 



prep_reflective_multiple <- create_filtered_wide_dataframes(prep_reflective, exposure_vars = "t1_household_inc_quantile") 



prep_social_multiple <- create_filtered_wide_dataframes(prep_social, exposure_vars = "t1_household_inc_quantile") 
...
#| label: test mi 
## TEST 
exclude_vars = c("t0_sample_frame", "id")
...
test_data <- mice(prep_health, m = 2, predictorMatrix = predictorMatrix)
...
test_health_alcohol_frequency  <- gcomp_sim(
...
# 
# impute_and_combine <- function(list_df, m = 2, exclude_vars = c("t0_sample_frame", "id")) {
...
# 
# t1 <- mice(prep_health_multiple[[1]], m = 10, predictorMatrix = predictorMatrix)
# t2 <- mice(prep_health_multiple[[2]], m = 10, predictorMatrix = predictorMatrix)
# t3 <- mice(prep_health_multiple[[3]], m = 10, predictorMatrix = predictorMatrix)
# t4 <- mice(prep_health_multiple[[4]], m = 10, predictorMatrix = predictorMatrix)
# 
# # Complete the data
# completed_t1 <- complete(t1, action = "long")
# completed_t2 <- complete(t2, action = "long")
# completed_t4 <- complete(t3, action = "long")
# completed_t3 <- complete(t4, action = "long") #, include = TRUE
# # Reset rownames of the completed dataframes
# rownames(completed_t1) <- NULL
# rownames(completed_t2) <- NULL
# rownames(completed_t3) <- NULL
# rownames(completed_t4) <- NULL
# 
# # Bind rows
# complete_df <- bind_rows(completed_t1, completed_t2, completed_t4, completed_t3)
# 
...
#   ungroup()
# 
# # Convert to a list of data frames for miceadds::datalist2mids
...
#mice_reflective_c  <- mice::complete(mice_reflective, "long", inc = T)
# 
...
# }
# # test
# mice_health_test <- mice(prep_health, m = 2, predictorMatrix = predictorMatrix)

# 
# mice_health_c_test  <- mice::complete(mice_health_test, action = 'long', include = TRUE)
# 
# 
# 
# vis_miss(mice_health_c_test, warn_large_data = FALSE)
# vis_miss(mice_health_c, warn_large_data = FALSE)
# 
# colnames( mice_health_c )
# table(mice_health_c$t1_household_inc_quantile)
# 
# row.names(mice_health_c_test) <- NULL
# 
# mice_health_mids_test <- mice_health_c_test %>%
#   rename(sample_weights = t0_sample_weights) |> 
#   mutate(t0_eth_cat = as.factor(t0_eth_cat),
#          t0_rural_gch2018 = as.factor(t0_rural_gch2018),
#          t0_smoker = as.factor(t0_smoker),
...
#   as.mids()
#   
#mice_health_long_test <- mice::complete(mice_health_mids_test, "long", inc = TRUE)
#skimr::skim(mice_health_long_test)
...
# checks
table(mice_embodied_c$t1_household_inc_quantile)
...
mice_practical_c  <- mice::complete(mice_practical, action = 'long', include = TRUE)

mice_practical_c <- mice_practical_c |> select(-c(.id.1, .imp.1))

table(mice_practical_c$.imp)

# check data
skimr::skim(mice_practical_c)

# check exposure
table(mice_practical_c$t1_household_inc_quantile)

row.names(mice_practical_c) <- NULL


mice_practical_mids <- mice_practical_c %>%
  arrange(.imp, id) |> 
  rename(sample_weights = t0_sample_weights) |> 
  mutate( t0_eth_cat = as.factor(t0_eth_cat),
         t0_rural_gch2018 = as.factor(t0_rural_gch2018)) |> 
...
# ebalance
dt_match_health_ebal <- match_mi(data = mice_health_mids, 
                                 X = X, 
                                 baseline_vars = baseline_vars_health, 
                                 estimand = "ATE",  
                                 method = "ebal", 
...
plot(sum_ebal_trim)
bal.tab(dt_match_health_ebal_trim)
...
plot(sum_ebal_trim)
bal.tab(mice_embodied_ebal_trim)
...
baseline_vars_practical = mice_practical_long |> 
  dplyr::select(starts_with("t0"), -c("t0_household_inc_z", "t0_nz_dep2018_z",
"t0_nzsei13_z", "t0_emotion_regulation_z"))|> colnames() # strange to include these -- as they  are income by other names
...
plot(sum_ebal_trim)
bal.tab(mice_practical_ebal_trim)
...
sum_ebal <- summary(mice_reflective_ebal)
sum_ebal
...
plot(sum_ebal_trim)
bal.tab(mice_reflective_ebal_trim)
