...
When these assumptions hold, causal inference in observational data becomes a more tractable problem, enabling the application of various estimation techniques to approximate the unobservable counterfactual outcomes.
...
This function computes the average effect by subtracting the average of the expected outcomes under two specific treatment levels $A$ and$A'$. Because the difference of the means is equivalent to the mean of the differences, this expression is equivalent to: 
...
We say there is an average or marginal effect of A on Y on the causal risk ratio scale if:
...
#### Introducing Conditional Average Treatment Effect (CATE)
