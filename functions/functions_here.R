...
prepare_data <- function(data, baseline_vars, exposure_var, outcome_vars) {
  # Filter the data and create necessary variables
  # ...
  
  # Return the cleaned data and selected variables
  list(
    data = cleaned_data,
...
# selected_data <- prepare_data(
#   data = dat,
#   baseline_vars = c("Age", "Edu", "EthCat", "Employed", "GenCohort", "NZDep2013", "NZSEI13", "Partner", "Parent", "Pol.Orient", "Rural_GCH2018", "SDO", "RWA", "AGREEABLENESS", "CONSCIENTIOUSNESS", "EXTRAVERSION", "HONESTY_HUMILITY", "OPENNESS", "NEUROTICISM"),
#   exposure_var = "nfd_became_17",
#   outcome_vars = c("Christian_nfd", "Religious")
# )
...
create_wide_data <- function(data, baseline_vars, exposure_var, outcome_vars) {
  # Add the 'time' column to the data
  data_with_time <- data %>%
    mutate(time = as.numeric(Wave) - 1)
  
  # Create the wide data frame
  wide_data <- data_with_time %>%
    select(-c(Wave, YearMeasured, Christian_nfd))  %>%
    pivot_wider(
      id_cols = Id,
      names_from = time,
    ...
    select(-c(starts_with("t0_") & matches(paste0("(", paste(exposure_var), ")$")) & (time > 0)))  %>% # only exposure year
    select(-c((starts_with("t1") | starts_with("t2") | starts_with("t3")) & matches(paste0("(", paste(baseline_vars), ")$")))) %>%
    select(-c((starts_with("t1") & matches(paste0("(", paste(outcome_vars), ")$")))))  %>%  # outcome vars only at baseline and outcome
