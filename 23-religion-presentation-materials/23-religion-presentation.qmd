...
push_mods <-
  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23_church_presentations"
  )
...
dat_full <- dat |>
  ...
  mutate(religion_church_binary2 = as.factor( ifelse(religion_church2 > 0, 1, 0))) |>
  mutate(religion_religious = as.factor (as.numeric(religion_religious) - 1) )|>
  dplyr::select(
    "wave",
    "year_measured",
    "id",
  ...
  filter(wave != 2009 &  wave != 2020 & wave !=2021) |> 
  dplyr::filter(year_measured == 1) |> 
  droplevels() |>
  rename(monthly_church = religion_church2,
         church_yes_no  =  religion_church_binary2,
         religious_yes_no = religion_religious) |> 
  mutate(monthly_church_factor = as.factor(round(ifelse(monthly_church >= 8, 8, monthly_church), 0)))


head(dat_full$religion_identification_level)

table1::table1(~ monthly_church |wave, dat_full, overall = FALSE)

table1::table1(~ church_yes_no |wave, dat_full, overall = FALSE)

table1::table1(~ monthly_church_factor |wave, dat_full, overall = FALSE)

table1::table1(~ religion_identification_level |wave, dat_full, overall = FALSE)

table1::table1(~ religious_yes_no |wave, dat_full, overall = FALSE)


dat_rel<-  dat_full |> 
  filter(religious_yes_no == 1)


table1::table1(~ monthly_church |wave, dat_rel, overall = FALSE)

table1::table1(~ church_yes_no |wave, dat_rel, overall = FALSE)

table1::table1(~ monthly_church_factor |wave, dat_rel, overall = FALSE)

table1::table1(~ religion_identification_level |wave, dat_rel, overall = FALSE)
...
table1::table1(~ monthly_church |alert_level_combined, data = alert_dat, overall = FALSE)

table1::table1(~ church_yes_no |alert_level_combined, alert_dat, overall = FALSE)

table1::table1(~ monthly_church_factor |alert_level_combined, alert_dat, overall = FALSE)

table1::table1(~ religion_identification_level |alert_level_combined, alert_dat, overall = FALSE)
...
hist(mice_donate_long$t0_total_siblings_z)
# we have insufficient overlap of religion variables. so we focus on religion
baseline_vars_donate_exposure
...
match_donate_ebal <- match_mi_general(data = mice_donate_mids, 
                                 X = X, 
                                 baseline_vars = baseline_vars_donate_exposure, 
                                 estimand = estimand,  
                                # focal = "zero",
                                 method = "ebal", 
                                 sample_weights = NULL)
...
love.plot(match_donate_ebal_trim, binary = "std", thresholds = c(m = .05)) # 
...
baseline_vars_warmth_exposure = mice_warmth_long |>
  dplyr::select(
    starts_with("t0"),
    -t0_religion_church_coarsen,
 #   -t0_alert_level_combined_lead,
...
mice_warmth_ebal <- match_mi_general(data = mice_warmth_mids, 
                                 X = X, 
                                 baseline_vars = baseline_vars_warmth_exposure, 
                                 estimand = estimand,  
                                # focal = "zero",
                                 method = "ebal", 
...
love.plot(mice_warmth_ebal_trim, binary = "std", thresholds = c(m = .05))
...
# use alert levels in iptw model
baseline_vars_support_exposure = mice_support_long |>
  dplyr::select(
    starts_with("t0"),
    -t0_religion_church_coarsen,
 #   -t0_alert_level_combined_lead,
...
# view
baseline_vars_support_exposure
...
match_support_ebal <- match_mi_general(data = mice_support_mids, 
                                 X = X, 
                                 baseline_vars = baseline_vars_support_exposure, 
                                 estimand = estimand,  
                                # focal = "zero",
                                 method = "ebal", 
                                 sample_weights = "sample_weights")
...
love.plot(match_support_ebal_trim, binary = "std", thresholds = c(m = .05))
...
exposure_varX
prep_donateX <- margot_wide(dat_long_tX, 
...
prep_warmthX <- margot_wide(dat_long_tX, 
  baseline_vars = baseline_varsX, 
  exposure_var = exposure_varX,
...
prep_supportX <- margot_wide(dat_long_tX, 
  baseline_vars = baseline_varsX, 
  exposure_var = exposure_varX,
...
prep_multiple_donateX <- margot_filter(prep_donateX, exposure_vars = exposure_varsX) 
prep_multiple_warmthX <- margot_filter(prep_warmthX, exposure_vars = exposure_varsX) 
prep_multiple_supportX <- margot_filter(prep_supportX, exposure_vars = exposure_varsX) 
...
# check ebal
sum_ebal <- summary(match_donate_ebalX)
sum_ebal
...
# # if trim is needed
match_donate_ebal_trimX <- WeightIt::trim(match_donate_ebalX, at = .99)
...
# graphs
plot(sum_ebal_trimX)
cobalt::bal.tab(match_donate_ebal_trimX, stats = c("m", "v"), binary = "std", thresholds = c(m = .05))
love.plot(match_donate_ebal_trimX, binary = "std", thresholds = c(m = .1))

# settings
# set dataframe
dt_donateX = match_donate_ebal_trimX
...
# if trim (if needed)
mice_warmth_ebal_trimX <- WeightIt::trim(mice_warmth_ebalX, at = .99)
sum_ebal_trimX <- summary(mice_warmth_ebal_trimX)
sum_ebal_trimX

# graphs
plot(sum_ebal_trimX)
bal.tab(mice_warmth_ebal_trimX,stats = c("m", "ks"), abs = TRUE)
love.plot(mice_warmth_ebal_trimX, binary = "std", thresholds = c(m = .1))
...
baseline_vars_supportX = mice_support_longX |> 
  dplyr::select(starts_with("t0"))|> colnames() # used for imputation -- fully co-linear
...
love.plot(match_support_ebalX, binary = "std", thresholds = c(m = .1))
...
# trim if needed
match_support_ebal_trimX <- WeightIt::trim(match_support_ebalX, at = .99)
...
love.plot(match_support_ebal_trimX, binary = "std", thresholds = c(m = .1))
