---
title: "Causal effects of prayer attendance on cooperation"
...
push_mods <-  fs::path_expand(
    "/Users/joseph/v-project\ Dropbox/data/nzvs_mods/00drafts/23-ow-coop-prayer"
  )
...
# check
# table( is.na(dat$religion_prayer) ) 
# table( is.na( dat$religion_prayer) )

# dat$religion_prayer
# table(dat$alert_level)
# hist(dat$religion_prayer)
# mean(dat$religion_prayer, na.rm = TRUE)
# sd(dat$religion_prayer, na.rm = TRUE)
# 
# ## prop total pop
# dat_pr <- dat |> filter( wave == 2018 & year_measured == 1) |>  filter(!is.na(religion_prayer))
# sum(dat_pr$religion_prayer ==0, na.rm=TRUE)/nrow(dat_pr)
# 
# ## prop religious
# dat_pr_rel <- dat |> filter( wave == 2018 & year_measured == 1 & religious ==1) |>  filter(!is.na(religion_prayer))
# sum(dat_pr_rel$religion_prayer >1, na.rm=TRUE)/nrow(dat_pr_rel)
# sum(dat_pr_rel$religion_prayer==0, na.rm=TRUE)/nrow(dat_pr_rel)
# sum(dat_pr_rel$religion_prayer>=8, na.rm=TRUE)/nrow(dat_pr_rel)
# 
# 
# # ## prop not religious
# dat_pr_rel_not <- dat |> filter( wave == 2018 & year_measured == 1 & religious ==0) |>  filter(!is.na(religion_prayer))
# nrow(dat_pr_rel_not)
# sum(dat_pr_rel_not$religion_prayer >0, na.rm=TRUE)/nrow(dat_pr_rel_not) # 2.3 in 100
# sum(dat_pr_rel_not$religion_prayer>7, na.rm=TRUE)/nrow(dat_pr_rel_not) # 4 in 1000
# sum(dat_pr_rel_not$religion_prayer, na.rm=TRUE)/nrow(dat_pr_rel_not)
# 
# dat_pr_rel_not <- dat |> filter( wave == 2018 & year_measured == 1 & religious ==0) |>  filter(!is.na(religion_prayer))
# nrow(dat_pr_rel_not)
# sum(dat_pr_rel_not$religion_prayer >0, na.rm=TRUE)/nrow(dat_pr_rel_not) # 2.3 in 100
...
dat_long <- dat |>
  ...
  dplyr::mutate(k_18 =  ifelse(wave == 2018 &
                                 !is.na(religion_prayer_binary) & !is.na(bigger_doms), 1, 0)) |>   # creating an indicator for the first wave
  dplyr::mutate(h_18 = mean(k_18, na.rm = TRUE)) |>   # Hack
  dplyr::mutate(k_19 =  ifelse(
    wave == 2019 &
      year_measured == 1 &
      !is.na(religion_prayer_binary),
    1,
...
table( dat_long$religion_prayer_binary )
...
#  filter(id %in% religious_ids) %>%   only if subsetting religious ids 
  ...
  mutate(
    religion_prayer_coarsen_n = as.numeric(religion_prayer_coarsen) - 1,
    religion_prayer_binary_n = as.numeric(religion_prayer_binary)
  ) |>
  mutate(
    religion_prayer_binary = as.factor(religion_prayer_binary),
...
baseline_vars = c(
   ...
   "religion_prayer_coarsen", # for prediction
...
exposure_var = c("religion_prayer_binary") 
...
## data for mice
exposure_vars = c("t1_religion_prayer_binary")
...
# Set exposure 
X <- "t1_religion_prayer_binary" # #levels( mice_donate_long$t1_religion_prayer_binary )
...
baseline_vars_donate = mice_donate_long |>
  dplyr::select(
    starts_with("t0"),
    -t0_religion_prayer_coarsen,
    -t0_religion_scripture_binary_z,
    -t0_religion_spiritual_identification_z
    -t0_bigger_doms
    -t0_religion|> colnames() # strange to include these -- as they  are income by other names
...
out <- msm::statetable.msm(religion_prayer_binary, id, data = dt_positivity_full)
#state_names <-c("zero", "one", "less_four", "four_up")
state_names <-c("prayer-no", "prayer-yes")
...
#| fig-cap: "Causal effects of prayer gain on virtue"
#| eval: true
#| include: true
#| echo: false
...
title = "ATE: +1 year causal effect from gain of prayer"
...
# ggsave(
#   group_plot_virtue,
#   path = here::here(here::here(push_mods, "group_plot_virtue_prayer_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtue-prayer-POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_virtue,
#   path = here::here(here::here("figs", "godin", "group_plot_virtue_prayer_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtue_prayer_POP",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
#| tbl-cap: "Causal effects of prayer gain on virtues"
#| eval: true
#| include: true
#| echo: false
...
#| fig-cap: "Causal effects of prayer gain on volunteering (y/n)"
#| eval: true
#| include: true
#| echo: false
### Risk variables
...
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rr-volunteers_prayer_population.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
#| tbl-cap: "Causal effects of prayer gain on revealed social support"
#| eval: true
#| include: true
#| echo: false
...
#| fig-cap: "Causal effects of prayer gain on warmth"
#| eval: true
#| include: true
#| echo: false
# donate continuou
...
# ggsave(
#   group_plot_ate_warmth_rd,
#   path = here::here(here::here(push_mods, "group_plot_ate_warmth_rd_prayer_POP")),
#   width = 8,
#   height = 4,
#   units = "in",
#   filename = "group_plot_ate_warmth_rd_prayer_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_ate_warmth_rd,
#   path = here::here(here::here("figs", "godin", "group_plot_ate_warmth_rd_prayer_POP")),
#   width = 8,
#   height = 4,
#   units = "in",
#   filename = "group_plot_ate_warmth_rd_prayer_POP.png",
...
#| tbl-cap: "Causal effects of prayer gain on warmth"
#| eval: true
#| include: true
#| echo: false
...
#| fig-cap: "Causal effects of prayer gain on perceived social support"
#| eval: true
#| include: true
#| echo: false
...
#title = "ATE: +1 year causal effect of prayer gain"
...
# ggsave(
#   group_plot_support_rd,
#   path = here::here(here::here(push_mods, "group_plot_support_rd_prayer_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rd_prayer_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_support_rd,
#   path = here::here(here::here("figs", "godin", "group_plot_support_rd_prayer_POP.png")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rd_prayer_POP.png",
...
#| tbl-cap: "Causal effects of prayer gain on perceived social support"
#| eval: true
#| include: true
#| echo: false
...
#| fig-cap: "Causal effects of prayer gain on revealed social support"
#| eval: true
#| include: true
#| echo: false
### Risk variables
...
# ggsave(
#   group_plot_ate_support_rr,
#   path = here::here(here::here(push_mods, "group_plot_ate_support_rr_prayer_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rr-prayer-POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_ate_support_rr,
#   path = here::here(here::here("figs","godin", "group_plot_ate_support_rr_prayer_POP")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rr_prayer_POP.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
dat_long_tX  <- dat_long %>%
  ...
  mutate(religion_prayer_coarsen_n = as.numeric(religion_prayer_coarsen) -
           1) |>
  mutate(
    religion_prayer_coarsen_n = as.numeric(religion_prayer_coarsen) - 1,
    religion_prayer_binary_n = as.numeric(religion_prayer_binary)
...
# set exposure variable, can be both the continuous and the coarsened, if needed
exposure_varX = c("religion_prayer_coarsen") # we could construct this after imputation. # "perfectionism_high" to be replace by "perfectionism_coarsen" - do the data wrangling after imputation.
...
## data for mice
exposure_varsX = c("t1_religion_prayer_coarsen")
...
levels( mice_donate_longX$t1_religion_prayer_coarsen)
# check
skim(mice_donate_longX)
...
#levels( mice_donate_longX$t1_religion_prayer_coarsen )
# Set exposure 
X <- "t1_religion_prayer_coarsen" #"t1_religion_prayer_coarsen" 
X
...
#levels(dat_long_t$religion_prayer_coarsen)
nsims = 200
#check treatments
#treat_0 = "zero"
# #check
...
out <- msm::statetable.msm(religion_prayer_coarsen, id, data = dt_positivity_fullX)
state_names <-c("zero", "one", "less_four", "four_up")
#state_names <-c("prayer-no", "prayer-yes")
...
titleX = "ATE: +1 year causal effect of +4 prayer gain (RELIGIOUS ONLY)"
...
# ggsave(
#   group_plot_virtueX,
#   path = here::here(here::here(push_mods, "group_plot_virtueX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtueX_prayer_REL.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_virtueX,
#   path = here::here(here::here('figs', "godin", "group_plot_virtueX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_virtueX_prayer_REL.png",
...
ggsave(
  gr_plot_virtue_rrX,
  path = here::here(here::here(push_mods, "gr_plot_virtue_rrX_prayer_REL")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "gr_plot_virtue_rrX_prayer_REL.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
# ggsave(
#   gr_plot_virtue_rrX,
#   path = here::here(here::here('figs', "godin",  "gr_plot_virtue_rrX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "gr_plot_virtue_rrX_prayer_REL.png",
...
#titleX = "ATE: +1 year causal effect of +4 prayer gain"
...
ggsave(
  group_plot_ate_warmth_rdX,
  path = here::here(here::here(push_mods, "group_plot_warmth_rdX_prayer_REL")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "group_plot_warmth_rdX_prayer_REL.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
# ggsave(
#   group_plot_ate_warmth_rdX,
#   path = here::here(here::here('figs', "godin",  "group_plot_warmth_rdX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_warmth_rdX_prayer_REL.png",
...
# ggsave(
#   group_plot_support_rdX,
#   path = here::here(here::here(push_mods, "group_plot_support_rdX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rdX_prayer_REL.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
# )
...
# ggsave(
#   group_plot_support_rdX,
#   path = here::here(here::here('figs', "godin",  "group_plot_support_rdX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_support_rdX_prayer_REL.png",
...
#| fig-cap: "Causal effects of prayer gain on revealed social supportX"
#| eval: true
#| include: true
#| echo: false
### Risk variables
...
ggsave(
  group_plot_ate_support_rrX,
  path = here::here(here::here(push_mods, "group_plot_ate_support_rrX_prayer_REL")),
  width = 8,
  height = 4.5,
  units = "in",
  filename = "group_plot_ate_support_rrX_prayer_REL.png",
  device = 'png',
  limitsize = FALSE,
  dpi = 600
)
...
#  ggsave(
#   group_plot_ate_support_rrX,
#   path = here::here(here::here('figs', "godin", "group_plot_ate_support_rrX_prayer_REL")),
#   width = 8,
#   height = 4.5,
#   units = "in",
#   filename = "group_plot_ate_support_rrX_prayer_REL.png",
...
dat_long_tX1  <- dat %>%
 # filter(id %in% religious_ids) %>%
  ...
  mutate(religion_religious = as.numeric(religion_religious) - 1) |>
  mutate(
    religion_prayer_round = round( ifelse(religion_prayer >=8, 8, religion_prayer), 0) )|> 
  mutate(
    eth_cat = as.integer(eth_cat),
    urban = as.numeric(urban),
    education_level_coarsen = as.integer(education_level_coarsen)
...
sd( dat_long_tX1$charity_donate, na.rm = TRUE)
mean(dat_long_tX1$charity_donate, na.rm = TRUE)


# save
saveRDS(dat_long_tX1, here::here(push_mods, "dat_long_tX1"))
dat_long_tX1<- here_read("dat_long_tX1")
...
# set exposure variable, can be both the continuous and the coarsened, if needed
exposure_varX1 = c("religion_prayer_round","not_censored") # 
...
df_clean_donate <- df_wide_censored_donate %>%
 ...
 # select(-t2_na_flag) %>%
  ...
  dplyr::mutate(across(where(is.numeric) &
                        !t0_not_censored & !t1_not_censored & !t0_sample_weights & !t1_religion_prayer_round & !t2_charity_donate & 
                         !t2_gratitude & !t2_hours_charity, ~ scale(.x), .names = "{col}_z"))|>
  select(where(is.factor), t0_not_censored, t1_not_censored, t0_sample_weights, t1_religion_prayer_round, t2_gratitude,  t2_charity_donate,t2_hours_charity, ends_with("_z")) |>
  relocate(starts_with("t0_"), .before = starts_with("t1_"))  %>%
  relocate(starts_with("t2_"), .after = starts_with("t1_"))  %>%
  relocate("t0_not_censored", .before = starts_with("t1_"))  %>%
  relocate("t1_not_censored", .before = starts_with("t2_")) |> 
...
# check: 
df_clean_donate$t1_religion_prayer_round
...
table(df_clean_donate$t1_religion_prayer_round)
...
m_t2_charity_donate_null <- here_read("m_t2_charity_donate_null")
...
# 1   209  1312 1103      35.1      140       277  <0.001
# calculate proportion 4 or greater
proportion = sum(df_clean_donate$t1_religion_prayer_round >= 8) / nrow(df_clean_donate)
proportion
