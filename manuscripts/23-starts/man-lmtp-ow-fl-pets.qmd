...
Focussing on the effect of cat ownership on sleep, the traditional causal estimand in which we contrast everyone getting cats verses no-one getting cats is `r out_tab_contrast_t2_hours_sleep_z_0` (outcomes are measured in standard deviation units).  The shift estimand in which contrast everyone getting cats with the natural state in which cat ownership does or does not occure is `r out_tab_contrast_t2_hours_sleep_z`.  As expected given the shift estimand proposes a less intrusive counterfactual intervention, we find the shift estimand provides is more conservative than the traditional all-or-nothing estimand. 
